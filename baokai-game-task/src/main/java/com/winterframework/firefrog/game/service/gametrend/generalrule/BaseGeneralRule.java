...
public class BaseGeneralRule {
	...
	private static final Logger log = LoggerFactory.getLogger(BaseGeneralRule.class);
	...
	//取上一期该类型的统计数据
	public GameTrendJbyl getLastGameJbylTrend(List<GameTrendJbyl> list) throws Exception {
	...
	//取上一期该类型的统计数据
	public GameTrendJbyl getLastGameJbylTrend_bk(List<GameTrendJbyl> list) throws Exception {
		GameTrendJbyl result = null;
		if (list != null) {
			for (GameTrendJbyl gameJbylTrend : list) {
				if (this.groupCode.longValue() == gameJbylTrend.getGameGroupCode().longValue()
				/*&& this.setCode.longValue() == gameJbylTrend.getGameSetCode().longValue()*/
				&& this.type.equals(gameJbylTrend.getTrendType())) {
					result = gameJbylTrend;
				}
			}
		}
		//数据为空，则使用初始值
	...
	public GameTrendJbyl getGameJbylTrend(String value, String viewValue, GameDrawResult gdr) throws Exception {
	...
	public List<Integer> getNumberRecordList(GameDrawResult gdr) {
		...
		try {
			list = numberRecordList.subList(arrBits[0], arrBits[1] + 1);
		} catch (Exception e) {
			log.error("getNumberRecordList error:lotteryId=" + gdr.getLotteryid() + "issue=" + gdr.getIssueCode()
					+ "record=" + gdr.getNumberRecord() + "arrBits=" + arrBits[1], e);
		}
		return list;
	}
	...
	*/
	public String makeNewOmission(String last, int v) {
		...
		for (int i = start; i < arr.length+start; i++) {
			Integer n = Integer.parseInt(arr[i-start]);
			if (v == i) {
				current.append("0,");
			} else {
				n++;
