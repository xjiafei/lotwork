...
public enum GameSlipUtilsEnum {
	...
	*/
	...
	public String[] getBetPackageDetai(Long lotteryid, GamePackageItem packitem, String[] contents) throws Exception {
		if (packitem.getFileMode() == 1) {
			File file = new File(packitem.getBetDetail());
			try {
			
				boolean is115 = lotteryid == 99301L
						|| lotteryid == 99302L
						|| lotteryid == 99303L
						|| lotteryid == 99304L
						|| lotteryid == 99305L
						|| lotteryid == 99306L
						|| lotteryid == 99307L;
				boolean isSSQ = lotteryid == 99401L;
				
				//3获取投注内容，
				String content = FileUtils.readFileToString(file);
				String methodCode = "";
				try {
					methodCode = packitem.getBetTypeCode().split("_")[2];
				} catch (Exception e) {
					log.error("获取methodCode失败slipid" + packitem.getId());
				}
				...
				String msg="获取文件投注信息错误，文件路径=" + packitem.getBetDetail();
				log.error(msg, e);
				//throw new Exception(msg,e);
				contents[0] = packitem.getBetDetail();
			}
		} else {
			contents[0] = packitem.getBetDetail();
		}
		log.debug("slip.getId()=" + packitem.getId() +"contents.length == "+contents.length +"----" + contents[0]);
		return contents;
	}
