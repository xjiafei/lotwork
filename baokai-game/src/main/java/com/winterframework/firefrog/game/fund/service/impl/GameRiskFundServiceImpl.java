...
public class GameRiskFundServiceImpl implements IGameRiskChangeFundGameVoService {
	...
	* @return
	...
	private GameFundServiceBean switchTypeToGameFundVos(TORiskDTO dto) throws Exception{
		...
		log.trace("switchTypeToGameFundVos users "+ users.length + ", Userid =" + dto.getUserid());
		log.trace("switchTypeToGameFundVos amounts "+ amounts.length +", Amount =" + dto.getAmount());
		...
		if(fundType!=GameFundTypesUtils.GAME_RET_UNFREEZER_DETEAIL_TYPE && fundType!=GameFundTypesUtils.GAME_CANCEL_RET_DETEAIL_TYPE
		   ...
		   && fundType!=GameFundTypesUtils.GAME_RET_FREEZER_DETEAIL_TYPE){
			FundGameVo fundGameVos = packFundVos(dto, users[0], amounts[0], fundType,reason,note,isAclUser,operator);
			log.trace("switchTypeToGameFundVos type ="+fundType+", users[0] ="+ users[0] + ", amounts[0]="+amounts[0]);
			...
			for (int i = 0; i < users.length; i++) {
				...
				FundGameVo fundGameVos = packFundVos(dto, users[i], amounts[i], fundType,reason,note,isAclUser,operator); 
				log.trace("switchTypeToGameFundVos type ="+fundType+", users[i] ="+ users[i] + ", amounts[i]="+amounts[i]);
				list.add(fundGameVos);
			} 
		}
	...
	private FundGameVo packFundVos(TORiskDTO dto, String user,
			String amount, int fundType,String reason,String note,Long isAclUser,Long operator) {
		FundGameVo fundGameVos = new FundGameVo();
		fundGameVos.setIsAclUser(isAclUser);
		fundGameVos.setOperator(operator);
		fundGameVos.setReason(reason);
		fundGameVos.setAmount(Long.parseLong(amount)); 
		fundGameVos.setUserId(Long.parseLong(user));
