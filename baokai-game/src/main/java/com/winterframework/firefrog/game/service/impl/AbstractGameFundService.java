...
public class AbstractGameFundService implements IGameFundService {
	...
	public void fundRequest(List<TORiskDTO> toRiskDTOList){
		...
		if(trace.length >= 4){
			log.info("审核中心开始请求资金系统 : Request From =>" 
					+ "Prev Step(1)= " + trace[2].getClassName() + "-" + trace[2].getMethodName()
					+ "Prev Step(2)= " + trace[3].getClassName() + "-" + trace[3].getMethodName()
					);
		}else {
			log.info("审核中心开始请求资金系统");
		}
		try {
			gameFundRiskServiceImpl.distributeAwardMMC(toRiskDTOList);
		} catch (Exception e) {
