 ...
 */
package com.winterframework.spring.web.servlet.handler;
...
public class DefaultRestErrorResolver implements RestErrorResolver, MessageSourceAware, InitializingBean {
	...
	private Response createResp(HttpServletRequest request, int errorCode) {
		try {
			...
			if (req == null) {
				req = new Request();
				req.setHead(new RequestHeader());
				errorCode = (errorCode == 0) ? 902 : errorCode;
			}
	...
	public Response resolveError(HttpServletRequest request, Object handler, Exception ex) {
		...
		String str = exceptionMappingDefinitions.get(ex.getClass().getName());
		if (str == null){
			RestError error=exceptionMappings.get(ex.getClass().getName());
			if(error!=null){
				str="200,"+error.getStatus();
			}
		}
		if(str==null){
			str="500,500";
		...
		return createResp(request, Integer.valueOf(value));
	}
