$(document).ready(function(){
	...
	var groupName = false,
	groupNameDom = $('#groupName'),pgid=$("#pid").val();
	$("#select").change(function(){
		var url = baseUrl+"/aclAdmin/buildAclTree";
		var pid = $("#select").val();
		var data = "id="+-1+"&pid="+pid+"&num=" + Math.random();
		$("#J-tree-container").empty();
		$("#J-tree-container2").empty();
	    jQuery.ajax({
			type: "post",
			url: url,
			data : data,
...
groupNameDom.blur(function(){
	...
	var url = baseUrl +"/aclAdmin/checkByName";
	 ...
	 $.ajax({
		...
		dataType: 'json',
		type: 'post',
		data:"name="+$.trim(groupNameDom.val()),
		success:function(data){
			if(data.status == 1){
				groupName = true;
			}else{
	...
	$('#J-button-submit').click(function(){
        ...
        var pid = $("#select").val();
        var data="&pid="+pid+"&inUser="+inUser+"&name="+encodeURI(title)+"&ids="+ids+"&userId=1"+"&num=" + Math.random();
