...
$(document).ready(function () {
    ...
    var scalePX = [31, 100, 243, 384, 525, 659]
    ...
    function showProgress(len) {
        ...
        var step = function () {
            ...
            if (width >= len) {
                setTimeout(function () {
                    if( bet>= scale[4]){
                        $float.css('left', -81 + width + 'px')
                    }else{
                        $float.css('left', -17 + width + 'px')
                    }

                    $float.show().html('<div id="float-info">当日累计销量</div><div><span>为</span><span id="float-sum">' + bet + '</span></div>')
		...
		if(signStatus == -22){//-22, "尚未報名"
            ...
            if(awardStatus == -3){//-3,預熱期
                ...
                $('#sign-up').bind('click',function(){
                        window.location  = "phlotto://go_lottolobby";    
                });
            }
		}
	}
	...
	console.log("mobile>>>"+source);
	   ...
	   $.ajax({
			url:'/activitymb/activity20170603signup',
			type:'POST',
			dataType:'json',
			cache: false,
			data:{source:source, month:201706, week:3,token : $("#token").val()},
    ...
    function processAward(){
        ...
        $.ajax({
            url:'/activitymb/activity20170603award',
            type:'POST',
            dataType:'json',
            cache: false,
            data:{source:source, month:201706, week:3,token : $("#token").val()},
