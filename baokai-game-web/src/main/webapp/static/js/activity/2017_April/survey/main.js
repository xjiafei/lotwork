    ...
    function countDown() {
        ...
        function startCount() {
            ...
            if (nowDate >= finalTime) {
				$.ajax({
					url:'/activity/activitystartaward',
					type:'POST',
					dataType:'text',
					async: false,
...
//弹窗居中
    ...
    $('#start-filling').bind('click', function () {
		...
		$.ajax({
			url:'/activity/activitycheckbettotal',
			type:'POST',
			dataType:'text',
			async: false,
		    success:function(data){
				var result = JSON.parse(data);
				if (result['isQualifi'] == 'true') {
					...
					$("#popup-info").html("<div>投注额满1000即可申请6元</div> <div>您还差<span style='color:#f8ff2c'>" + result['betDifference'] + "</span>元哦</div>")
				}
				positionFixd('#popup');
				$("#popup").show()
				$('#mask').show()
