function signUp(){
	...
	$.ajax({
		url:'/activitymb/signup20161203',
		type:'POST',
		dataType:'json',
		cache: false,
		 data:{source:source, month:201612, week:3, token : $("#tokenValue").val()}, 
...
function querySignUp(){
	...
	$.ajax({
		url:'/activitymb/querysignup20161203',
		type:'POST',
		dataType:'json',
		cache: false,
		data:{source:source, month:201612, week:3, actId:20161203,token : $("#tokenValue").val()},
		success:function(data){
			if(data == 0 ){   //可以報名
...
if (oJoin) {
	oJoin.addEventListener('click', function(){
	  signUp();
	  ga('send', 'event', '12月第三周活动报名', '点击【立即参加】按钮');
	}, false);
}
