function signUp(){
	...
	$.ajax({
		...
		dataType:'json',
		cache: false,
		data:{source:source, month:201612, week:4, token : $("#tokenValue").val()}, 
...
function querySignUp(){
	...
	$.ajax({
		...
		dataType:'json',
		cache: false,
		data:{source:source, month:201612, week:4, actId:20161204,token : $("#tokenValue").val()},
		success:function(data){
			if(data == 0 ){   //可以報名
