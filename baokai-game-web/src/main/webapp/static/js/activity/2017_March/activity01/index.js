$(function(){
  ...
  function rewardAnimation() {
    ...
    $('.J-hongbao').fadeIn()
      .find('.mid')
      .find('.bless').css({ "background-position": '0 ' + random + 'px'}).end()
  ...
  $(document).on('click', '.J-reward', function(){
        ...
        //$this.addClass('disabled');
         ...
         $.ajax({
           url: '/activity/activity20170301award',
           type: 'post',
		   cache: false,
           dataType: 'json',
			  ...
			  if (data.awardStatus == '4'){
				...
				timer = setInterval(function(){
				  ...
				  if(data.status == '-6'){
					  ...
					  showPop({success: 3, message: '今日充值金额未达标', link: '/fund'});
				  } else if(data.status == '-8') {
					  showPop({success: 4, message: '今日流水未达标', link: (game_server + '/gameBet/cqssc/')});
				  } else if(data.status == '3') {
					  showPop({success: 4, message: '明日可领取奖金', link: (game_server + '/gameBet/cqssc/')});
				  } else if(data.status == '-4'){
					  showPop({success: 2, message: '活动将于3月6日开始，敬请期待', link: '/index'});
  ...
  $(document).on('click', '.J-chongzhi', function() {
    ga('send', 'event', '3月份第一周活动页[鸿运储值]', '点击鸿运储值');
  });
