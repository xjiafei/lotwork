...
$(document).ready(function () {
	 ...
	 $('#welcome').removeClass('notSelected').addClass('notStarted');	
		...
		console.log('已報名');
			if(awardStatus == -3){
				...
				console.log('已報名 activity20170501rate');
	...
	console.log('signStatus:'+signStatus)
	 ...
	 $('#sign-up').bind('click', function () {
	   ...
	   $.ajax({
			...
			url:'/activity/activity20170501signup',
			type:'POST',
			dataType:'json',
			cache: false,
			data:{source:source, month:201705, week:1},
	...
	function processAward(){
				...
				$.ajax({		   
					url:'/activity/activity20170501rate',
					type:'POST',
					dataType:'json',
					cache: false,
					success:function(data){															
					...
					console.log(award);
					$.ajax({		   
						url:'/activity/activity20170501award',
						type:'POST',
						dataType:'json',
						cache: false,
						data:{bet:bet, rate:rate, multiple:multiple, award:award},
