$(function() {
	...
	rewards.prototype = {
		...
		getData: function(defer) {
			var me = this;
			$.ajax({
					url: '/sheepYear/rewards?userId='+userid+'&device='+userdevice,
					type: 'GET',
					cache:false,
					dataType: 'json'
				})
			...
			warp.find('.rp-button').click(function() {
				$.get('/sheepYear/rewards', {
					...
					if (Number(res['status']) === 0) {
						...
						warp.addClass('rp-step-' + num).html(me.getHtml(num));
						me['action' + num](warp, num, currentNum, res);
					}
				}, 'json');
			});
		},
		...
		action3: function(dom, step, i, res) {
			...
			dom.html(me.getHtml(3, data)).removeClass('rp-step-6').addClass('rp-step-7');
		...
		action5: function(dom, step, i, res) {
			var me = this,
				...
				data = res['data'],
				process = Number(data['lastBet']) / (Number(data['expected']) / 100),
				deadline = data['deadline'],
				nowtime = data['nowTime'];

			data['rewardsNum'] = me.formatNum(data['rewardsNum']);
			data['lastBet'] = me.formatNum(data['lastBet']);
