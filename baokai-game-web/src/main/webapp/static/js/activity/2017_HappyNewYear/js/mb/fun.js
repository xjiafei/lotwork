...
$(function(){
	$.ajax({
        url:'/activitymb/newyeargroupdate',
        dataType: 'JSON',
        cache: false,
        type:'post',
        ...
        success:function(data){
				...
				if(fastActivity == 2){
					...
					$('.btn.recharge').attr("href", "phlotto://go_recharge");
				}
				if(thirdActivity == 2){
					$(".startRed").removeClass("notStart");
				} else if (thirdActivity == 1) {
					$(".startCharge").attr("href", "phlotto://go_recharge");
				}
            }
    });
})
...
$(function(){
	...
	if(fastActivity != 0){
		...
		//活动一数据初始化
		$.ajax({
        url:'/activitymb/newyearchargedoubleinit',
        dataType: 'JSON',
        cache: false,
        type:'post',
		data: {token:$("#token").val()},
	...
	$('.btn.getmoney').on('click',function(){	
		$.ajax({
			url:'/activitymb/newyearchargedoublegetaward',
			dataType: 'JSON',
			cache: false,
			type:'post',
			data: {token:$("#token").val()},
	...
	function startMarquee(){
		var $ul = $('.winnerList ul'),
            ...
            num = 0;
        if($li.length>3){
            $ul.css({'width': length*2*$li.width()});
            $ul.append($li.clone());
            function moving(){
                $ul.css({'marginLeft': 0});
                $ul.stop().animate({'marginLeft': -$li.outerWidth()*length},3000*length,'linear',function(){
                    moving();
                });
            }
            moving();
	...
	if(twoActivity != 0){
		...
		$.ajax({
		url:'/activitymb/newyearlotteryinit',
		dataType: 'JSON',
		cache: false,
		type:'post',
		data: {token:$("#token").val()},
    ...
    $('.startMove').on('click',function(){
        ...
        $.ajax({
            url:'/activitymb/newyearlotterylottery',
            dataType: 'JSON',
            cache: false,
            type:'post',
			data: {token:$("#token").val()},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
                    var $dom = $('.act2');
                    $dom.find('.picMove').css({"width": 14.41*7+"rem","left":0});
                    $dom.find('.picMove').animate({"left":(-14.41*5-1.31*(prizeMap[data.prize]-2))+"rem"},4000,"swing",function(){
	...
	function initThirdActive(){
		$.ajax({
		url:'/activitymb/newyearchargegiveinit',
		dataType: 'JSON',
		cache: false,
		type:'post',
		async:false,
    ...
    $('.startRed').on('click',function(){
        ...
        $.ajax({
            url:'/activitymb/newyearchargegiveredbowaward',
            dataType: 'JSON',
            cache: false,
            type:'post',
			data: {sn:$("#chargeSn").val(),token:$("#token").val()},
			async:false,
            success:function(data){
				$(".act3p1 .obtain").html(data.todayRedBow);
				$(".obtainLeft").html(data.todayNowRedBow);
    ...
    $('#yearEvePacket').on('click',function(){
        var $this = $(this)
        $.ajax({
            url:'/activitymb/newyearvipredbowsave',
            dataType: 'JSON',
            cache: false,
            type:'post',
			data:{activityCode:"2017_CNY_4-1",token:$("#token").val()},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
    ...
    $('#yearPacketLv1').on('click',function(){
        ...
        $.ajax({
            ...
            cache: false,
            type:'post',
			data:{activityCode:"2017_CNY_4-2",token:$("#token").val(),newyeartype:1},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
                    //var $result = $('<div class="result">恭喜获得<h2>'+data.result +'</h2></div>');
                    //$this.html($result);
    ...
    $('#yearPacketLv2').on('click',function(){
        ...
        $.ajax({
            ...
            cache: false,
            type:'post',
			data:{activityCode:"2017_CNY_4-2",token:$("#token").val(),newyeartype:2},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
                    //var $result = $('<div class="result">恭喜获得<h2>'+data.result +'</h2></div>');
                    //$this.html($result);
    ...
    $('#yearPacketLv3').on('click',function(){
        ...
        $.ajax({
            ...
            cache: false,
            type:'post',
			data:{activityCode:"2017_CNY_4-2",token:$("#token").val(),newyeartype:3},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
                    //var $result = $('<div class="result">恭喜获得<h2>'+data.result +'</h2></div>');
                    //$this.html($result);
    ...
    $('#yearPacketLv4').on('click',function(){
        ...
        $.ajax({
            ...
            cache: false,
            type:'post',
			data:{activityCode:"2017_CNY_4-3",token:$("#token").val()},
            success:function(data){
                if(data.status == 1){           //表示获取数据成功
                    //var $result = $('<div class="result">恭喜获得<h2>'+data.result +'</h2></div>');
                    //$this.html($result);
