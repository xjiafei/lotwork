...
var ptgame_server = "http://pt.taojirnke.com/pt/index/";
var fh_server = "http://www.taojirnke.com/index";
$(function(){
	//-------------总奖池----------------------
	//格式化金额
	...
	(function(){
		...
		function slide(index){
			...
			if(supportCss3('transform-style')) {
				...
				oPics.eq(historyIndex).find('.btn_start').hide();
				oPics.eq(historyIndex).delay(600).hide(100);
				oPics.eq(index).find('.flip').addClass('in').removeClass('out');
				oPics.eq(index).find('.btn_start').hide().delay(500).fadeIn();
			}else{
	...
	$('#pop-login .tab li').click(function(){
	...
	$("#login-confirm").click(function(){
		...
		var loginType = $("#J-input-loginType").val();
		var str = '{"userName":"'+userName+'","passWord":"'+passWord+'","type":"login","loginType":"'+loginType+'"}';
		var data = JSON.parse(str);
...
$(function(){
	...
	var submitForm=function()
	{
		...
		var ps2 = encodeURIComponent(password2.val());
		if(isOpenVcode()){
			if(!checkVcode()){
				return;
			}
			params = {'username':username.val(),'password1':ps,'password2':ps2,
				vcode:vcode.val(),'device':'web','pt':'y'};
		}else{
			params = {'username':username.val(),'password1':ps,'password2':ps2,'device':'web','pt':'y'};
		}
