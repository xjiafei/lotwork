$(document).ready(function() {
	...
	//查寻后select回选
	$('#J-select-lotteryid').val($.trim($('#lotteryid').val()));
	$('#J-select-time').val($.trim($('#selectTimeMode').val()));
	$('#J-time-start').val($.trim(formatDate($('#startCreateTime').val())));
	$('#J-time-end').val($.trim(formatDate($('#endCreateTime').val())));
	$('#J-select-status').val($.trim($('#status').val()));
	...
	if ($('#startCreateTime').val() != '') {
		inputStart.val(formatDate(Number($('#startCreateTime').val())));
	}
	if ($('#endCreateTime').val() != '') {
		inputEnd.val(formatDate(Number($('#endCreateTime').val())));
	}
	if ($('#issueAward').val() != '') {
	...
	$('#J-download-submit').click(function(){
		...
		if (selectLotteryId.val() != '') {
			$('#lotteryid_').val(selectLotteryId.val());
		}else{
			$('#lotteryid_').val(null);
		}
		if (inputParamCode.val() != '') {
			$('#paramCode_').val(inputParamCode.val());
		}else{
			$('#paramCode_').val("");
		}
		...
		if(selectTime.val() != ''){
			...
			$('#selectTimeMode_').val(mode);
			
			if(mode == -1){
				$('#startCreateTime_').val("");
				$('#endCreateTime_').val("");
			}else{
				if (inputStart.val() != '') {
					var time = convertDate2UnixTimestamp(inputStart.val());
					$('#startCreateTime_').val(time);
				}
				if (inputEnd.val() != '') {
					var time = convertDate2UnixTimestamp(inputEnd.val());
					$('#endCreateTime_').val(time);
				} 
			}
		}
		if(inputIssueAward.val() != ''){
			$('#issueAward_').val(inputIssueAward.val());
		}else{
			$('#issueAward_').val(null);
		}
		if(inputIssueWinsRatio.val() != ''){
			$('#issueWinsRatio_').val(inputIssueWinsRatio.val());
		}else{
			$('#issueWinsRatio_').val(null);
		}
		if(inputBetAward.val() != ''){
			$('#betAward_').val(inputBetAward.val());
		}else{
			$('#betAward_').val(null);
		}
		if(inputBetWinsRatio.val() != ''){
			$('#betWinsRatio_').val(inputBetWinsRatio.val());
		}else{
			$('#betWinsRatio_').val(null);
		}
		if(inputWinsTime.val() != ''){
			$('#winsTime_').val(inputWinsTime.val());
		}else{
			$('#winsTime_').val(null);
		}
		if(selectStatus.val() != ''){
			$('#status_').val(selectStatus.val());
		}else{
			$('#status_').val(null);
		}
		...
		if(dom.hasClass('button-noaudit')){
			Wd.show({
				...
				confirmFun:function(){
					...
					{
						$.ajax({
							url:baseUrl + '/gameRisk/auditGameRiskOrder?ids='  + orderId + '&status=1' + '&disposeMemo='+$('textarea').val(), 
							dataType:'json',
