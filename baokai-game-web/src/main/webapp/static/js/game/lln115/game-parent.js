		...
		exeEvent_ball:function(param, target){
			...
			if(Games.getCurrentGame().getCurrentGameMethod().getGameMethodName() == 'quwei.normal.dingdanshuang') {
				...
				$.each($el,function(n,v){
					...
					arr.push(Number(Games.getCurrentGame().specialMultiple[param.value]));
				});
				Games.getCurrentGameStatistics().setMultipleLimit(arr.sort(function(a,b){return a-b})[0]);
...
(function(host, name, Event, undefined){
	var defConfig = {
		...
		//追号数据列表模板
		dataRowTemplate:'<tr><td class="text-center"><#=No#></td><td><input data-action="checkedRow" class="trace-row-checked" type="checkbox" checked="checked"> <span class="trace-row-number" issueCode=<#=issueCode#>><#=traceNumber#></span></td><td><input class="trace-row-multiple" value="<#=multiple#>" type="text" style="width:30px;text-align:center;"> 倍</td><td><span class="price"><dfn>&yen;</dfn><span class="trace-row-money"><#=money#></span> 元</span> </td><td><span class="trace-row-time"><#=publishTime#></span></td></tr>',
		//高级追号盈利金额追号/盈利率追号表模板
