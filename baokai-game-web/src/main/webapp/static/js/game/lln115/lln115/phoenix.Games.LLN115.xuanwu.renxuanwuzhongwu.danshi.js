...
(function(host, Danshi, undefined){
	var defConfig = {
		name:'xuanwu.renxuanwuzhongwu.danshi',
		//玩法提示
		tips: '',
		//选号实例
		exampleTip: '',
		exampleText: '03 04 06 08 11<br />04 05 06 10 11 <br />01 04 05 07 10  <br />01 06 08 09 10  <br />01 03 06 09 11 '
	},
	Games = host.Games,
	...
	var pros = {
		...
		checkBallIsComplete:function(data){
			...
			for(i = 0;i < result.length;i++){
				...
				//判断单注规则
				if(/^\d{2}[\s]\d{2}[\s]\d{2}[\s]\d{2}[\s]\d{2}$/.test(current)
					&& !(me.checkBallGroup(saveArray))
					&& !(me.checkArrayInnerSame(saveArray))){
					current = [saveArray[0], saveArray[1], saveArray[2], saveArray[3], saveArray[4]].sort(function(a, b){
						return a-b;
					})
