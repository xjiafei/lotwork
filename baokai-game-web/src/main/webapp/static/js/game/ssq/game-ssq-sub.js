...
(function(host, name, Game, undefined){
	var defConfig = {
		//游戏名称
		name:'SSQ',
		basePath:staticFileContextPath + '/static/js/game/ssq/',
...
(function(host, name, GameMethod, undefined){
	var defConfig = {
		...
		uploadButton: '#file',
		//单式导入号码示例
		exampleText: '06,11,21,25,28,33+02<br />08,18,23,24,28,30+09<br />04,16,24,26,28,32+04<br />03,18,22,25,26,32+05',
		...
		normalTips : '<p style="color:#888;font-size:12px;line-height:170%;">' + ['说明：',
					...
					'2、红球之间的间隔符支持 逗号[,] 句号[.]，红球蓝球之间的间隔符支持 分号[;] 冒号[:] 竖线[|]加号[+],每注之间的间隔符支持 回车 空格[ ]',
					'3、文件格式必须是.txt格式。',
	...
	var pros = {
		...
		removeOrderError: function(){
			...
			for (var i = 0; i < me.vData.length; i++) {
				result += me.vData[i][0].join(',') + '+' + me.vData[i][1] + '<br>';
			};
			me.errorDataTips();
			if($.trim(result) == ''){
				me.showNormalTips();
		...
		removeOrderSame: function(){
			...
			for (var i = 0; i < me.aData.length; i++) {
				...
				result += $.isArray(me.aData[i]) ?  me.aData[i][0].join(',') + '+' + me.aData[i][1] + '<br>' : me.aData[i] + '<br>';
			}
			me.sameDataTips();
			me.replaceText(result);
			me.checkBallIsComplete(result);
		...
		sameDataTips: function(){
			...
			for (var i = 0; i < sameData.length; i++) {
				sameData[i] = $.isArray(sameData[i]) ? sameData[i][0].join(',') + '+' + sameData[i][1] : sameData[i];
			};
