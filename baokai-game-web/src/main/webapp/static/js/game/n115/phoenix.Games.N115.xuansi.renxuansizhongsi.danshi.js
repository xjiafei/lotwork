...
(function(host, Danshi, undefined){
	var defConfig = {
		name:'xuansi.renxuansizhongsi.danshi',
		//玩法提示
		tips: '',
		//选号实例
		exampleTip: '',
		//标准格式样本
		exampleText: '03 04 06 08<br />04 05 06 10<br />01 04 05 07<br />01 06 08 09<br />01 03 06 09'
	},
	Games = host.Games,
	...
	var pros = {
		...
		checkBallIsComplete:function(data){
			...
			for(i = 0;i < result.length;i++){
				...
				//判断单注规则
				if(/^\d{2}[\s]\d{2}[\s]\d{2}[\s]\d{2}$/.test(current)
					&& (Number(saveArray[0]) > 0 && Number(saveArray[0]) < 12)
					&& (Number(saveArray[1]) > 0 && Number(saveArray[1]) < 12)
					&& (Number(saveArray[2]) > 0 && Number(saveArray[2]) < 12)
					&& (Number(saveArray[3]) > 0 && Number(saveArray[3]) < 12)
					&& (Number(saveArray[0]) != Number(saveArray[1]))
					&& (Number(saveArray[0]) != Number(saveArray[2]))
					&& (Number(saveArray[0]) != Number(saveArray[3]))
					&& (Number(saveArray[1]) != Number(saveArray[2]))
					&& (Number(saveArray[1]) != Number(saveArray[3]))
					&& (Number(saveArray[2]) != Number(saveArray[3]))){
					current = [saveArray[0], saveArray[1], saveArray[2], saveArray[3]].sort(function(a, b){
						return a-b;
					})
