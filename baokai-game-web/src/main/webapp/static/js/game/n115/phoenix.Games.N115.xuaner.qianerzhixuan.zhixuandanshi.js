...
(function(host, Danshi, undefined){
	var defConfig = {
		name:'xuaner.qianerzhixuan.zhixuandanshi',
		//玩法提示
		tips: '',
		//选号实例
		exampleTip: '',
		//标准格式样本
		exampleText: '03 04<br />04 05<br />01 04 <br />01 06 <br />01 03'
	},
	Games = host.Games,
	...
	var pros = {
		...
		createRandomNum: function(){
			...
			for(var k=0;k < len; k++){
				...
				if(arr[ranDomNum] < 10){
					current[0] = [$.trim(current[0] + ' 0' + arr[ranDomNum])];
				}else{
					current[0] = [$.trim(current[0] + ' ' + Number(arr[ranDomNum]))];
				}
		...
		checkBallIsComplete:function(data){
			...
			for(i = 0;i < result.length;i++){
				...
				if(/^\d{2}[\s]\d{2}$/.test(current)
					...
					current = [saveArray[0], saveArray[1]].join(' ');
