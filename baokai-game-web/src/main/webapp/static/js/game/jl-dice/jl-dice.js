;(function($){
	...
	F.prototype = {
		...
		createBalance: function(){
			var html =
				'<div class="dice-balance">' +
					'<div class="bet-amount"><label>下注额:</label><span class="value_box">￥<span data-count-amount>0.00</span></span></div>' +
					'<div class="bet-balance"><label>余额:</label><span class="value_box">￥<span data-user-balance>21,500,000.00</span></span>' +
						'<i class="ui-icon bet-balance-refresh" data-action="refresh">刷新</i>' +
						'<a class="ui-icon bet-balance-recharge" data-action="recharge" href="javascript:;">充值</a>' +
					'</div>' +
				'</div>';			
		...
		createActionButtons: function(){
			...
			// 下注
			html += '<a href="javascript:;" class="ui-button ui-button-primary button-bet" data-action="bet">下注</a>';
			// 撤销
			html += '<a href="javascript:;" class="ui-button button-revocation" data-action="revocation">撤销</a>';
			// 清空
			html += '<a href="javascript:;" class="ui-button button-clear" data-action="clear">清空</a>';

			html += '</div>';


			this.$bar.append(html);
			return this.$actions = this.$bar.find('a[data-action]');
            //
			////richardgong 2015-05-13
            //
			// this.$bar.append($("#new-action-buttons"));
			// return this.$actions = this.$bar.find('button[data-action]');
		},
		// 生成投注历史开关按钮
		createHistoryToggle: function(){
			var html = 
				'<div class="history-toggle">投注记录 \
					<!--<span>投注记录</span> \
					<i></i>--> \
				</div>';
			this.$bar.append(html);
		...
		setGameLimit: function(limit){
			...
			$.each(gamelimit, function(i,n){
				...
				if('hezhi'===i){
					me.gamelimit['k3hezhiMultiple'] = n['k3hezhiMultiple'];
				}
			});
		...
		// 获取中奖列表
		getWinningList: function(data,callback){
			var me = this;
			$.ajax({
				url: me.gameConfig.getWinningListUrl(),
		...
		chipIsOverLimit: function($t, val){
			var me = this,
				...
				val = parseFloat(val) || 0;
				//骰寶和值拿這組
				array=gamelimit['k3hezhiMultiple'];
			var limit = gamelimit[data['name'] || ''] || 0,
				value = parseFloat(data['value']) || 0;
			if(data['name']=='hezhi')
			{   
		        
				gamelimit.hezhi=array[order];
				limit=array[order]*2;
				/*
		...
		afterSubmitSuccess: function () {
			...
			try {
				...
				$.ajax({
					dataType: 'json',
					cache: false,
					url: me.gameConfig.getUserOrdersUrl(),
					cache: false,
					success: function (re) {
						if($.isEmptyObject(re)){
							return;
						}else{
							me.gameHistory.cleanHtml();
							me.gameHistory.append(re);
						}
					}
				});
			} catch (err) {
		...
		showBetPrize: function(arr){
			var me = this;
			$.each(arr,function(i,v){
				me.$sheets.eq(Number(v)).addClass('dice-sheet-hover');
			});
			setTimeout(function(){
				me.hideBetPrize();
			},5000)
		},
		hideBetPrize: function(){
			this.$sheets.removeClass('dice-sheet-hover');
		},
		/**
		 * currency(num, n, x) * 
