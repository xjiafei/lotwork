...
(function(host, name, Game, undefined){
	var defConfig = {
		//游戏名称
		name:'llssc',
		basePath:staticFileContextPath + '/static/js/game/llssc/llssc/',
								...
								success:function(data){
									if(data['status'] == '1'){								
										Msg.show({
											mask:true,				
											title:'温馨提示',				
											content:'<div class="text-center"><div class="pop-title"><i class="ico-success"></i><h4 class="pop-text">奖金组配置成功!	<br>刷新当前游戏页面。</h4></div></div>',
											closeIsShow :true,	
											hideClose: true,
											closeFun:function(){ 
												 location.reload();                  
 ...
 //        'name':'player'
	...
	var pros = {
		init: function(cfg){
			...
			me.sources=phoenix.Games.LLSSC.Config.getInstance().defConfig.sourceList;
			//me.getSourceList();
			me.bindEvent();
		},
		setSourceList:function(arr){
			this.sources=arr;
		},
		//绑定按钮事件
		getSourceList: function(){
			this.sources = [
				'src=rtmp://103.17.41.44/live/livestream&streamType=live&configuration=rtmp://103.17.41.44/live/livestream&initialBufferTime=5&idx=1',
				'src=rtmp://103.17.41.44/live/livestream&streamType=live&configuration=rtmp://103.17.41.44/live/livestream&initialBufferTime=5&idx=2',
				'src=rtmp://103.17.41.44/live/livestream&streamType=live&configuration=rtmp://103.17.41.44/live/livestream&initialBufferTime=5&idx=3'
			];
		},
		buildVideo: function(){
			var me = this,
				flashvars = me.sources[me.selectedSourceIdx] || 'src=rtmp://103.17.41.44/live/livestream&streamType=live&configuration=rtmp://103.17.41.44/live/livestream&initialBufferTime=5';
		...
		bindEvent: function(){
			...
			/*videoContainerDom.bind('mousemove', function(){
...
$(function(){
	var video = new phoenix.Games.LLSSC.Video();
});
