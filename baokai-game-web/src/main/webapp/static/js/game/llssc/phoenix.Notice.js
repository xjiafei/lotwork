...
(function(host, name, Event, $, undefined){
	...
	var pros = {
		...
		messageCycleStop: function(name, html){
			...
			if(html && html.find('li').is(':animated')){
				html.find('li').stop();
			}
		...
		addTipsContainerDom: function(){
			...
			me.tipsContainer = $('<ul class="game_tipscontainer ie6fixedBR"></ul>');
...
$(function(){
	...
	var msgNoticeTips = new phoenix.Notice({
		delayTime: 2,
		serviceUrl: '/static/js/json/msgNotice.json'
	});
	...
	var msgTips = new phoenix.Notice({
		delayTime: 5,
		serviceUrl: '/static/js/json/msgTipsPush.json',
		serviceGetType: 'push',
		msgPushWaitTime: 8
	});
