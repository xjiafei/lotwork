				...
				success:function(result){
					if(Number(result['isSuccess']) == 1){
						...
						if(rebateSelect&&retPoint > 0){
							...
							rebateSelect.setValue(actualBonus+' - ' + per +'%');
							...
							$('#J-select-rebate').html('<option selected="selected" retPointValue="2"  value=""></option>');
...
(function(host, name, Event, undefined) {
	var defConfig = {
		...
		//每行投注记录html模板
		rowTemplate: '<li data-param="action=reselect&id=<#=id#>" id="gameorder-<#=id#>"><div class="result"><span class="retPoint"><#=retPointText#></span><span style="font-family: Arial,verdana;"><span ><dfn>&yen;</dfn></span><#=amountText#><#=moneyUnitText#></span><span class="bet"><#=num#>注</span><span class="multiple"><#=multiple#>倍</span><span class="close"><a data-param="action=del&id=<#=id#>" href="javascript:void(0);">删除</a></span></div><span>[<#=typeText#>]</span><span style="position:relative" ><#=lotterysText#></span></li>',
		//显示内容截取字符串长度
	...
	var pros = {
		...
		selectOrderById: function(id) {
			...
			//设置奖金返点模式
			if (typeof rebateSelect !== 'undefined') {
			...
			for (; i < times; i++) {
				...
				if (multipleBase > maxMultiple) {
					Games.getCurrentGameMessage().show({
						...
						closeText: '确定',
						closeFun: function() {
							me.trace_advanced_yinglilv_b(maxMultiple);
							me.updateStatistics();
							this.hide();
						},
						data: {
                    ...
                    success: function (re) {
						if($.isEmptyObject(re)){
							...
							$.each(re, function (i) {
								...
								htmls += "<td></td><td><span class='price'><dfn>&yen;</dfn>" + me.formatMoney(re[i].totamount / 10000) + "</span></td><td></td>";
