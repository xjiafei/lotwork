...
(function(host, GameMethod, undefined){
	...
	var pros = {
		...
		//生成后端参数格式
		makePostParameter: function(data){
			var me = this,
				oddNum = [],
				evenNum = [],
				data = data['lotterys'],
			...
			for (; i < data.length; i++) {
				for (var j =0; j < data[i].length; j++) {
					if(me.arrIndexOf(data[i][j], data[i]) == 4){
						if(me.arrIndexOf(data[i][j], evenNum) == -1){
							evenNum.push(data[i][j]);
						}
					}else{
						if(me.arrIndexOf(data[i][j], oddNum) == -1){
							oddNum.push(data[i][j]);
						}
					}
				}
			}
			return evenNum.join('') + ',' +oddNum.join('');
		},
		//获取组合结果
		getLottery: function(){
			var me = this,
