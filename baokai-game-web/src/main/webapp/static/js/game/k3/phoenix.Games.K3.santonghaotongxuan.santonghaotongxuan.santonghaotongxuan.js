...
(function(host, GameMethod, undefined){
	var defConfig = {
		name:'santonghaotongxuan.santonghaotongxuan.santonghaotongxuan',
		//玩法提示
		tips:'三同号通选',
		//选号实例
		exampleTip: '三同号通选玩法范例'
	},
	Games = host.Games,
	...
	var pros = {
		...
		makePostParameter: function(original){
			...
			for(;i < len;i++){
				result.push('111 222 333 444 555 666');
			}
	...
	var html_row = [];
			...
			html_row.push('</div>');
			html_row.push('<ul class="ball-content santonghaotongxuan">');
			$.each([0,1], function(i){
				if(i==0){
					html_row.push('<li style="display:none"><a href="javascript:void(0);" data-param="action=ball&value='+ this +'&row=<#=row#>" class="ball-number">三同号通选</a><span class="ball-aid-hot">0</span></li>');
				}else{
					html_row.push('<li><a href="javascript:void(0);" data-param="action=ball&value='+ this +'&row=<#=row#>" class="ball-number">三同号通选</a><span class="ball-aid-hot">0</span></li>');			
				}
			});
			html_row.push('</ul>');
