...
(function(host, GameMethod, undefined){
	var defConfig = {
		name:'santonghaodanxuan.santonghaodanxuan.santonghaodanxuan',
		//玩法提示
		tips:'三同号单选',
		//选号实例
		exampleTip: '三同号单选玩法范例'
	},
	Games = host.Games,
	...
	var html_row = [];
			...
			html_row.push('</div>');
			html_row.push('<ul class="ball-content santonghaodanxuan">');
			$.each([0,1,2,3,4,5,6], function(i){
				if(i==0){
					html_row.push('<li style="display:none"><a href="javascript:void(0);" data-param="action=ball&value='+ this +'&row=<#=row#>" class="ball-number">' + this + '' + this + '' + this + '</a><span class="ball-aid-hot">0</span></li>');
				}else{
					html_row.push('<li><a href="javascript:void(0);" data-param="action=ball&value='+ this +'&row=<#=row#>" class="ball-number">' + this + '' + this + '' + this + '</a><span class="ball-aid-hot">0</span></li>');			
				}
			});
			html_row.push('</ul>');
