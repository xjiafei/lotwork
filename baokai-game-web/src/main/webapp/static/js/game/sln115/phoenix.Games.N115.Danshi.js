...
(function(host, name, GameMethod, undefined){
	...
	var pros = {
		...
		_bulidEditDom: function(){
			...
			me.doc.writeln('<html>'+headHTML+'<body style="word-break: break-all;color:#F5EEEE;">' + me.getNormalTips() + '</body></html>');
			me.doc.close();
			// //FOCUS光标	
			// if(!document.all){
			// 	me.win.focus();
		...
		getHTML:function(){
			...
			var html_all = [];
				html_all.push('<div class="balls-import clearfix">');
					html_all.push('<form id="form1" name="form1" enctype="multipart/form-data" method="post" action="./simulatedata/insertPreMoney.php" target="check_file_frame" style="position:relative;padding-bottom:10px;">');
					...
					html_all.push('<input type="button" class="balls-import-input" value="" onclick=document.getElementById("form1").file.click()><a class="balls-example-danshi-tip" style="color:#dbb453;" href="#">查看标准格式样本</a>');
					html_all.push('<iframe src="./simulatedata/insertPreMoney.php" name="check_file_frame" style="display:none;"></iframe>');
