...
(function (host, name, Event, undefined) {
    $('.ball_number_area').append('<div class="ball_number_area_border">');
    var defConfig = {
            //主面板dom
            ...
            //每行投注记录html模板 richardgong 2016-3-18
            rowTemplate: '<li data-param="action=reselect&id=<#=id#>" id="gameorder-<#=id#>"><div class="result"><span class="retPoint"><#=retPointText#></span><span class="moneyUnitText"><#=moneyUnitText#></span><span class="bet"><#=num#>注</span><span class="multiple"><#=multiple#>倍</span><span class="price"><span>&yen;</span><#=amountText#></span><span class="color-gray">不支持加注</span><span class="close"><a data-param="action=del&id=<#=id#>" href="javascript:void(0);" title="删除">删除</a></span></div><span>[<#=typeText#>]</span><span><#=lotterysText#></span></li>',
            diamondrowTemplate: '<li data-param="action=reselect&id=<#=id#>" id="gameorder-<#=id#>"><div class="result"><span class="retPoint"><#=retPointText#></span><span class="moneyUnitText"><#=moneyUnitText#></span><span class="bet"><#=num#>注</span><span class="multiple"><#=multiple#>倍</span><span class="price"><span>&yen;</span><#=amountText#></span><input type="checkbox" value="1" class="plus-price" /> <span class="diamond-tips plus_diamond_input" data-tips="<#=diamondgradexplain#>"> 加注 (<#=diamondprice#>)</span><span class="close"><a data-param="action=del&id=<#=id#>" href="javascript:void(0);" title="删除">删除</a></span></div><span>[<#=typeText#>]</span><span><#=lotterysText#></span></li>',
            diamondrowTemplatechecked: '<li data-param="action=reselect&id=<#=id#>" id="gameorder-<#=id#>"><div class="result"><span class="retPoint"><#=retPointText#></span><span class="moneyUnitText"><#=moneyUnitText#></span><span class="bet"><#=num#>注</span><span class="multiple"><#=multiple#>倍</span><span class="price"><span>&yen;</span><#=amountText#></span><input type="checkbox" value="1" checked="checked" class="plus-price" />  <span class="diamond-tips plus_diamond_input plus_diamond_on" data-tips="<#=diamondgradexplain#>">加注 (<#=diamondprice#>)</span><span class="close"><a data-param="action=del&id=<#=id#>" href="javascript:void(0);" title="删除">删除</a></span></div><span>[<#=typeText#>]</span><span><#=lotterysText#></span></li>',
            //显示内容截取字符串长度
    ...
    var pros = {
        ...
        add: function (order) {
            ...
            if (order['lotterys'] && order['lotterys'].length > 0) {
                ...
                //奖金组
                var rebateSelect = phoenix.Games.getCurrentGameStatistics().rebateSelect;
                if (rebateSelect) {
                ...
                order.isdiamond = window.isdiamond;
            ...
            me.addPluseAlldiamond(order);


            var diamondtips = $('.diamond-tips');
            diamondtips.each(function () {
                var src = $('#diamond_pop_rotary').attr('src');

                var str = '<div class="tipscontentsssss"><div class="titlesss">加注中奖后将开启钻石转盘,根据模式选定转盘,奖金将依转盘转中结果进行倍数成长！<a href="/gameActive/mmc/diamondHelpguilder" target="_blank" class="mq">&nbsp;&nbsp; <i class="icons">?</i>了解更多</a></div><img src="'+src+'"><table cellpadding="0"cellspacing="0">';

                str += ("<tr><td>钻一转盘</td><td>钻二转盘</td><td>钻三转盘</td><td>钻四转盘</td></tr></table</div>");

                $(this).qtip({
                    content: {
                        text: str // Use the "div" element next to this for the content
                    },
                    position: {
        ...
        addPluseAlldiamond: function (order) {
            var me = this;
            var $containerDom = $(this.defConfig.containerDom);
            var checkbox = $containerDom.find('.plus-price');
            var betBtn = $('.bet-btn');
            if(this.plusealldiamond) return;

            if (!this.plusealldiamond) {

                this.plusealldiamond = true;
                $('#J-submit-plus').click(function (e) {
                    if($(this).hasClass('disabled')) return;
                    if($.cookie('isShowDiamondTips')){
                        var checkbox = $containerDom.find('.plus-price');
                        var allplus = $containerDom.find('.plus_diamond_input');

                        checkbox.prop('checked', true);
                        allplus.addClass('plus_diamond_on');
                        checkbox.each(function () {
                            me.setDiamondPluse(this);
                        });
                        $('#J-submit-order').trigger('click');
                        ...
                        Games.getCurrentGameMessage().show({
                            ...
                            confirmFun: function () {
                                ...
                                $('#J-submit-order').trigger('click');
                                if(this.getContentDom().find('.isShowDiamondTips').is(':checked')){
                                    $.cookie('isShowDiamondTips',true);
                                }
                                this.hide();
                            },
                            cancelIsShow: true,
                            ...
                            data: {
                                tplData: {
                                    msg: '确认全部加注并投注<br><label><input class="isShowDiamondTips" type="checkbox">不在提示</label>'
                                }
                            }
                        });
                    }
                ...
                $('.rechoose_ball').click(function () {
                    var checkbox = $containerDom.find('.plus-price');
                    //var all = me.plusealldiamondparent.find('input');
                    checkbox.prop('checked', false);
                    //all.is(':checked') ? checkbox.prop('checked', true) : checkbox.prop('checked', false);
                    //checkbox.each(function () {
                    //    me.setDiamondPluse(this);
                    //});
                });
            ...
            $('#J-balls-order-container').on('click','.diamond-tips',function(){
                var a = $(this).prev();
                a.is(':checked') ? $(this).removeClass('plus_diamond_on'):$(this).addClass('plus_diamond_on');
            ...
            // if ($('#J-balls-order-container').find('.plus-price').size()) {
        ...
        selectOrderById: function (id) {
            ...
            Games.getCurrentGameStatistics().getMultipleDom().setValue(order['oldMultiple']);
