...
;(function () {
    ...
    //     if (start > -1) {
    //         var morenumber = (start + len + 1) - strlength;
    //         if (morenumber > 0) {
    //             var last = ''
    //             for (var i = 0; i < morenumber; i++) last += "0";
    //             str = str + last;
    //         }
    //
    //         return str.substr(0, (start + len + 1));
    //     } else {
    //         var last = '.'
    //         for (var i = 0; i < len; i++) last += "0";
    //         return (str + last);
    //     }
    // }
    phoenix.util.formatMoney = function (num) {
        var num = Number(num),
            re = /(-?\d+)(\d{3})/;
    ...
    function Lottery() {
        ...
        this.dialogMarkup = {
            ...
            'result': '<div class="dialog" id="prize_result"> \
							<div class="result_bg"></div> \
							<div class="result_cont">总计<span class="highlight" data-result>23,456,789.00</span>元</div> \
						</div>',
    ...
    Lottery.prototype = {
        ...
        dialogStyle: function (type, $t) {
            ...
            } else if (type == 'result' || type == 'noprize') {
                $target = me.$header;
                css.top = $target.offset().top + 30;
                /*css = {
    ...
    var template = function (tpl, data) {
        var me = this,
            o = data,
            p, reg;
        if (data.issuportdiamond) {
            data.diamondText = '<span>加注(+' + (data.diamondAmount / 10000).toFixed(2) + ')</span>';
        } else {
            if (data.methodName) {
                var methodName = (data.methodName).substr(0, 2);
                if (methodName == '五星' || methodName == '前三' || methodName == '前二') {
                    data.diamondText = '<span class="color-gray">不支持加注</span>';
                } else {
                    data.diamondText = '<span>未加注</span>';
                }
            }
        }
    ...
    (function (host, Event, name, undefined) {
        var defConfig = {
                ...
                $gameMenu: '#J-top-game-menu,#J-user-center,#J-msg-panel',
                //等待开奖html模版 diamondText
                waitBeting: ['<li>',
                    '<div class="result">',
                    '<span class="retPoint"><#=retAward#></span>',
        ...
        SUBMIT.getSubmitData = function (e) {
            ...
            for (; i < len; i++) {
                ...
                result['balls'].push({
                    ...
                    // 'retPoint': ballsData[i]['retPointText'],
                    'num': ballsData[i]['num'],//传递投注数
                    'hasdiamondSubmit': ballsData[i]['isCheckBox']
                });
            }
            ...
            SUBMIT.submitData = function (e) {
                ...
                if (data.balls.length <= 0) {
                    ...
                    $('#J-submit-plus').removeClass('disabled');
                    return;
                }

                me.fireEvent('beforeSubmitSuccess');
                mmcBetKey = $.trim($('#mmcBetKey').val());
                $.cookie('mmcBetCookie99112', mmcBetKey, {
                    expires: 1,
                    path: '/',
                ...
                window.hasdiamondSubmit = !!$('#J-balls-order-container .plus-price:checked').size();
                $.ajax({
                    ...
                    success: function (r) {
                        if (Number(r['isSuccess']) == 1) {
                            var obj =r.data;
                            obj.diamondLv>0 && $('#imgRotate').attr('src',$('.preloadimg img').eq(obj.diamondLv-1).attr('src'));
        ...
        var pros = {
            init: function (cfg) {
                ...
                SUBMIT.addEvent('afterSubmitSuccess', function (e, r) {
                    ...
                    if (me.getSaveCompletePeriod() == 1) {
                        ...
                        if (betData['continuousPeriod'] > 1) {
                            ...
                            me.defConfig.$submitButton.parent().removeClass('bet-diamond');
                        }
                    }
                ...
                $('.bet-btn').addClass('bet-diamond').append('<input type="button" id="J-submit-plus" value="" class="diamond_plus_all">');

                $('body').diamondsBoard().init();
            ...
            disabledSubmitButton: function () {
                ...
                $('#J-submit-plus').addClass('disabled');
            },
            ...
            getSingleOpeningHtml: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i]['retPoint'] > 0) {
                        data[i]['retAward'] = data[i]['awardMode'] > 1 ? (data[i]['groupAward'] + data[i]['retAward']) / 10000 + ' - ' + '0.00%' : data[i]['groupAward'] / 10000 + ' - ' + data[i]['retPoint'] / 100 + '%';
                    } else {
                        data[i]['retAward'] = '';
                    }
            ...
            singlefinish: function (r) {
                ...
                lottery.flipball().flip(data['result'], true, function () {
                    ...
                    if (Number(data['winMoney']) > 0 && Number(data['winNum']) > 0) {
                        if (me.isShowRotate(data)) {
                            me.beforShowRotate(data);
                            $('.bg-plate').show();
                            var pStar =$("#plateStar");
                            pStar.find('strong').text('');
                            setTimeout(function () {
                                timedown(data, function () {
                                    lottery.dialog('show', 'result', {
                                        'result': SUBMIT.formatMoney(data['winMoney'] / 10000)
                                    });
                                    $('.bg-plate').hide();
                                    pStar.removeClass('plate-star2');
                                    isFinishedStar = true;
                                    theend();
                                });
                            }, 0);

                            pStar.off('click').click(function () {
                                var self = this;
                                pStar.off('click');
                                time = 0;
                                plateStar(data, function () {
                                    lottery.dialog('show', 'result', {
                                        'result': SUBMIT.formatMoney(data['winMoney'] / 10000)
                                    });
                                    $('.bg-plate').hide();
                    ...
                    function theend() {
                        ...
                        $('.refreshBall').click();
                        //显示收集钻石
                        Games.collectDiamonds.showPrize(r.data['result'].split(','));

                        //更新排行榜列表
                        $('body').diamondsBoard().afterSubmit();
                    }
            ...
            beforShowRotate: function (data) {
                var classname = ['plate-title-1', 'plate-title-2', 'plate-title-3', 'plate-title-4'];
                $('.plate-title').removeClass(classname.join(' ')).addClass(classname[(data.diamondLv - 1)]);
                $('#platePop').hide();
                $('#plateStar strong').text(3);
                var rst = data.result.split(',');
                var diamond = rst[0];
                var li = $('.plate-nums li');
                li.removeClass();
                li.each(function (i) {
                    if (diamond == rst[i]) {
                        $(this).addClass('ball-diamonds-' + rst[i]);
                    } else {
                        $(this).addClass('ball-' + rst[i]);
                    }
                });


                var tpl = '<li><span>[<#=methodName#>]</span><span><#=code#></span></li>';
                var html = '';
                var winMoney = 0;
                for (var key in data.list) {
                    var cur = data.list[key];
                    if (cur.diamondWin > 0) {
                        winMoney += cur.winMoney / 10000;
                        html += this.formatRowses(tpl, cur);
                    }
                }
                $('.plate-order').empty().html(html);

                $('#plateBonus').text(formatMoney(winMoney));


            },
            formatRowses: function (tpl, order) {
                var o = order,
                    p, reg;
                for (p in o) {
                    if (o.hasOwnProperty(p)) {
            ...
            continuousfinish: function () {
                ...
                me.defConfig.$submitButton.parent().addClass('bet-diamond');
                //打开提示状态
                me.openWaringStatus();
            ...
            //多次开奖过程
            //其中单次开奖
            isShowRotate: function (data) {
                for (var key in data.list) {
                    var cur = data.list[key];
                    if (cur.diamondWin > 0) {
                        return data.diamondLv > 0;
                    }
                }
                return false;
            },
...
var rotateFunc = function (angle, awards, callback, data) {  //awards:奖项，angle:奖项对应的角度
    $('#imgRotate').stopRotate();
    $("#imgRotate").rotate({
        angle: 0,
        duration: 5000,
        animateTo: angle + 1440, //angle是图片上各奖项对应的角度，1440是我要让指针旋转4圈。所以最后的结束的角度就是这样子^^
        callback: function () {
            bonusFunc(awards, data, callback);
        }
    });
};
function formatRowses(tpl, order) {
    var o = order,
        p, reg;
    for (p in o) {
        if (o.hasOwnProperty(p)) {
...
var bonusFunc = function (awards, data, callback) {
    ...
    for (var key in data.list) {
        ...
        if (cur.diamondWin > 0) {
            winMoney += cur.winMoney / 10000;
            bouns += cur.diamondWin / 10000;
            html += formatRowses(tpl, cur);

        }
    }
    $('.bonus-single,#plateBonus').text(formatMoney(winMoney));
    $('#platePop').fadeIn();
    $('.bonus-multiple').text('x ' + data.diamondTimes);
    $('.bonus-total').text(formatMoney(bouns));
...
var plateStar = function (obj, callback) {
    isFinishedStar = false;
    $('#plateStar').addClass('plate-star2');
    time = 3;
    $("#plateStar strong").html('');
    
    var data = {
        1: {2: [292.5, 203], 2.5: [158.5], 3.5: [114.5], 1.8: [67.5], 5: [23.5], 1.5: [-20.5], 3: [-113.5]},
        2: {2: [-21.5], 3: [-65.5], 2.5: [-111.5, -203.5], 5: [-158.5], 4.5: [-248.5], 3.5: [-294.5], 8: [-338.5]},
        3: {10: [21], 3: [-22], 6: [-69], 5.5: [-113], 4: [-158], 5: [-201], 4.5: [-250], 3.5: [-293]},
        4: {12: [21], 4.5: [-22], 6.5: [-69], 5: [-113], 7: [-158, -247], 6: [-201], 4: [-293]}
    }[obj.diamondLv][obj.diamondTimes];

    var f = ['-', '+'][Math.floor(Math.random() * 2)];
    var f2 = '' + Math.floor(Math.random() * 20);
    var s = +(f + f2);
    var arg = data[Math.floor(Math.random() * data.length)] + s;

    
    rotateFunc(arg, obj.diamondTimes, callback, obj);
}


var time = 3;
var isFinishedStar = true;
function timedown(num, callback) {
    if (isFinishedStar) {
        if (time == 0) {
            plateStar(num, callback);
        } else {
            $("#plateStar strong").html(time);
            time--;
            setTimeout(function () {
                timedown(num, callback);
            }, 1000);
        }
    }
}
