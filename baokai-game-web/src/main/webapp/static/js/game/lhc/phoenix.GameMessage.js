		...
		rebulidpauseBet : function(parameter){
			...
			result['confirmFun'] = function(){
				var order = Games.getCurrentGameOrder(),
					...
					arr = [],
					newType = '';
				if(parameter['data']['tplData']['balls'][i]['type']){
					arr = parameter['data']['tplData']['balls'][i]['type'].split('.');
					arr.pop();
					newType = arr.join('.');
					...
					for (; i < parameter['data']['tplData']['balls'].length; i++) {
						...
						order.removeData(parameter['data']['tplData']['balls'][i]['id'],newType)
					};
					//提交投注
			...
			parameter['data']['tplData']['msg'] = function(){
				...
				for (; k < parameter['data']['tplData']['balls'].length; k++) {
					...
					var plus = arr.pop();
					current = arr.join('.');
					var typeText = gameConfig.getTitleByName(current,plus);
			...
			$('#J-blockade-panel-main').on('click', '[data-action]', function(e){
				...
				switch(action){
					...
					case 'blockade-detail' :
						//将投注内容转换成Input内容
						var form = $('#J-form-blockade-detail').attr("action",Games.getCurrentGame().getGameConfig().getInstance().submitUrl()+"/showBlockadeInfoLhc"),
							splitStr = '-';
						form.html('');
						//选球内容和玩法名称以 /// 分隔
						$.each(balls, function(){
