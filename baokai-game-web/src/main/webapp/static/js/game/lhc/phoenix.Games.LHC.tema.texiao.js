...
(function(host, GameMethod, undefined){
  ...
  var pros = {
    ...
    buildOdds: function(arr){
        var me = this;
        var $dom = me.container.find('.texiao').find(defConfig.ballOddDom);
    ...
    updateHistory: function(){
      var me = this;
      var orders = Games.getCurrentGameOrder().getTotal().orders['tema.texiao'];
      if (!orders){ return false; }
      defConfig.selectArr = $.map(orders, function(obj){
        return obj.content;
      });
      var historyItem = me.container.find('.lottery-history').find('li').find('.ball-special');
      $.each(historyItem, function(i, obj){
        var $this = $(obj);
        if ($.inArray($this.text(), defConfig.selectArr) !== -1) {
      ...
      $.each(lotteryHistory, function(i, obj){
        ...
        $.each(numberRecord, function(i, val){
          if(i == 6){
            ...
            html_history.push('<span class="ball-special">'+val+'</span>');
          } else {
  ...
  var html_shengxiao = [];
  //生肖
  html_shengxiao.push('<div class="ball-list texiao">');
  html_shengxiao.push('<div class="game-title"><h3>特肖</h3>');
  ...
  $.each(defConfig.texiaoMap,function(i,v){
      var arrBall = [];
      ...
      html_shengxiao.push('<li data-param="action=ball&row=0&value='+ v['value'] +'&type=texiao"><span class="balls">'+v['cnName']+'</span>');
