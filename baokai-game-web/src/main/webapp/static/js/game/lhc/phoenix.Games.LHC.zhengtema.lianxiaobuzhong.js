...
(function(host, GameMethod, undefined){
  ...
  var defConfig = {
    ...
    name: 'zhengtema.lianxiaobuzhong',
  ...
  var pros = {
        ...
        var me = this,
           datas = cfg.getGameOdds();
			...
			for(var i = 0; i < datas.length ; i++){
				...
				if(datas[i].name=='lianxiaobuzhong'){
					odds=childs;
				}
			}
		//rebuild odds
    ...
    buildOdds: function(odds){
        var me = this,
            $dom = me.container.find('.lianxiaobuzhong').find(me.defConfig.oddDom),
            odds_list = [],
    ...
    setTips: function(cfg){
        ...
        $.each(data,function(i,v){
            if(v['name'] == 'lianxiaobuzhong' ){
				$.each(v.childs, function(i, v){
				  var $domRule = me.container.find('.lianxiaobuzhong').find(ruleDom),
					  $domExample = me.container.find('.lianxiaobuzhong').find(exampleDom);
					  $domRule.html(v['rule']);
    ...
    updateHistory: function(){
      ...
      if(betType == 'dan'|| betType == 'tuo'){
       // orders = defConfig.preSelect[playType]['dan'].concat(defConfig.preSelect[playType]['tuo']);
	      orders = preSelect[playType]['dan'].concat(preSelect[playType]['tuo']);
      }
    ...
    buildHistory: function(lotteryHistory){
      ...
      $(html_all).appendTo(me.container.find('.lianxiaobuzhong'));
    }
  }
  ...
  //html模板
  var html_lianxiaobuzhong = [];
  html_lianxiaobuzhong.push('<div class="ball-list lianxiaobuzhong clearfix">');
  html_lianxiaobuzhong.push('<div class="game-title">');
  html_lianxiaobuzhong.push('<h3>连肖(不中)</h3>');
  html_lianxiaobuzhong.push('<div class="game-prompt">');
  html_lianxiaobuzhong.push('<a class="rule-example" href="javascript:;">玩法规则</a>');
  html_lianxiaobuzhong.push('<a class="prompt-example" href="javascript:;">投注示例</a>');
  html_lianxiaobuzhong.push('<div class="rule-text"><span class="box-arrow"></span><p class="text-box"></p></div>');
  html_lianxiaobuzhong.push('<div class="prompt-text"><span class="box-arrow"></span><p class="text-box"></p></div>');
  html_lianxiaobuzhong.push('</div>');
  html_lianxiaobuzhong.push('</div>');
  html_lianxiaobuzhong.push('<div class="game-odds2">');
  html_lianxiaobuzhong.push('<ol>');
  html_lianxiaobuzhong.push('<li class="active">四不中<br>&times;1.8</li>');
  html_lianxiaobuzhong.push('</ol>');
  html_lianxiaobuzhong.push('</div>');
  ...
  $.each(defConfig.zodiacsMap,function(i,v){
      ...
      html_ball.push('<li data-param="action=preSelect&row=0&value='+ v['value'] +'&type=lianxiaobuzhong"><span class="balls">'+v['cnName']+'</span>');
  ...
  //拼接所有
  var html_all = [];
  html_all.push(html_lianxiaobuzhong.join(''));
