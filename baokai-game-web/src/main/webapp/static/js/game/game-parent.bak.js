		...
		_bulidHTMl: function(data, type,othername) {
			...
			var  otherclass = othername || '';	
				switch(type){
					...
					case 'top':
					//richardgong 2015-12-4
					title.append('<li class="'+data.name+otherclass+'">' + data.title.replace('超级2000,','') + '</li>');
					...
					case 'terminal':
					setTimeout(function(){
						result.find('.' +  data.mode + ' .' + data.parent).append('<dd class="'+data.name+otherclass+'">'+data.title+'</dd>');
					},0);
					break;
				}
		},
