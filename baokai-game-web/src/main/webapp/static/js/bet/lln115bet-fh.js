$(document).ready(function(){
	...
	var Games = phoenix.Games;
		//游戏实例
		phoenix.Games.LLN115.getInstance();
		//游戏玩法切换
		phoenix.GameTypes.getInstance();
		//统计实例
		phoenix.GameStatistics.getInstance({moneyUnit:$.cookie("moneyUnitDom-record") || 1});
		//号码篮实例
		...
		//消息类
		phoenix.Games.LLN115.Message.getInstance();
	...
	var mask = phoenix.Mask.getInstance(),		
		...
		try
		{
			...
			$('#J-trace-builddetail').css("font-size","15PX");
			if($.trim(isVideo) == '1'){
				//自动打开视频
				$('.lottery-video').click();
			}			
					...
					.done(function(data) {	
						if(data){		
							...
							if(data['lastballs']){												
								...
								$('#J-lotter-info-lastnumber').html(data['lastnumber']);	
								//上期开奖号码
								$('#J-lottery-info-balls').find('em').each(function(i){
									this.innerHTML = lastballs[i];
								});	
