$(document).ready(function() {
	...
	$(document).on("click","a[id^=subUserId_]",function(){
		var userId=$(this).attr("id").substr(10);
		if ($('#J-date-start').val() != '') {
			var time = convertDate2UnixTimestamp($('#J-date-start').val());
			$('#jdate').val(time);
		}
		var url="querySubUserReport?userId="+userId+'&account='+$('#account').val()+'&lotteryId='+$("#lotteryId").val()+'&betTypeCode='+$("#betTypeCode").val()+'&queryTime='+$('#jdate').val();
		location.href = url;
	});
...
function doForward(index) {
	if (index == -1) {
		if($.trim($("#forwardPage").val()) == ''){ return false;}
