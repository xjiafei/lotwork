$(document).ready(function() {
	...
	if ($('#jdate').val() != '') {
		$('#J-date-start').val(formatDate(new Date(Number($('#jdate').val()))));
	}
	...
	$('#curClick').click(function(){
		if ($('#J-date-start').val() != '') {
			var time = convertDate2UnixTimestamp($('#J-date-start').val());
			$('#jdate').val(time);
		}
		var url="querySubUserReport?userId="+$('#curUserId').val()+'&account='+$('#account').val()+'&lotteryId='+$("#lotteryId").val()+'&betTypeCode='+$("#betTypeCode").val()+'&queryTime='+$('#jdate').val();
		location.href = url;
	});
