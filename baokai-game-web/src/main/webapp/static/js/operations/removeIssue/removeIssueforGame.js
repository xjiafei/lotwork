$(document).ready(function(){
	...
	jQuery.ajax({
			...
			success: function(data){
				$.each(data,function(key,val){
						checkRemoveDate(val.lotteryId,val.gameTime);		
					});
			},
			complete:function(){
	...
	function checkRemoveDate(lotteryId,gameTime){
		...
		if(gameTime != null){
			var strTime = gameTime.split("T");
			var ymd = strTime[0].split("-");
			var mis = strTime[1].split(":");
			var removeDate = new Date(ymd[0],ymd[1]-1,ymd[2],mis[0],mis[1],mis[2]).getTime();
			
			if(timeZone >= removeDate && gameTime != "close" ){
				$(".game-menu-list a[href$="+lotteryName[lotteryId]+"]").hide();
				$("a[href$="+lotteryName[lotteryId]+"\\/]").parent().hide();
				$("li[data-link="+lotteryName[lotteryId]+"]").hide();
			}
		}
	}	
	});