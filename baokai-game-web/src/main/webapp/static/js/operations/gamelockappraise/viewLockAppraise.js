(function($){
	...
	PriceTable.prototype.initDom = function(){
		...
		for(i = 0, len2 = data.length; i < len2; i++){
			tdStr = [];
			tdStr.push('<tr class="tr-row"><td><span class="row-step">'+ (me.type == 'up' ? len2 - i : (i+1)) +'阶段</span></td><td><span class="money-start">'+ (data[i][0][0].toFixed(2) == 0.00 ? space : '')  + data[i][0][0].toFixed(2) +'</span> 至 '+ data[i][0][1].toFixed(2) +'</td><td>'+ data[i][1] +' %</td>');
