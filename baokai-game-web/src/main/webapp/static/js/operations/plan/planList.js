$(document).ready(function() {
	...
	var Wd = phoenix.Message.getInstance(),
		...
		selectStopMode = $('#J-select-trace'),
		inputStopParams = $('#J-input-stopX'),
	...
	if ($('#stopMode').val() != '') {
		selectStopMode.val($('#stopMode').val());
	}
	if ($('#stopParams').val() != '') {
		inputStopParams.val($('#stopParams').val());
	}
	if ($('#pageCount').val() != '') {
	...
	//setTimeFn(1);
	$('#J-select-trace').change(function(){
		var v = this.value;
		if(v == '1'){
			$('#J-money-trace').show();
		}else{
	...
	$('#J-button-submit').click(function(){
		...
		if(selectStopMode.val() != ''){
			if(selectStopMode.val()==2 || selectStopMode.val()==0){
				$('#stopMode').val(selectStopMode.val());
				$('#inputStopParams').val("");
				$('#stopParams').val("");
			}else if(selectStopMode.val() == -1){
				$('#stopMode').val("");
				$('#inputStopParams').val("");
				$('#stopParams').val("");
			}else{
				$('#stopMode').val(selectStopMode.val());
				if(inputStopParams.val() != ''){
					$('#stopParams').val(inputStopParams.val());
				}
			}
		}
	...
	$('#J-button-export').click(function(){
		...
		$('#Span-day').html($('#J-select-time option:selected').text());
