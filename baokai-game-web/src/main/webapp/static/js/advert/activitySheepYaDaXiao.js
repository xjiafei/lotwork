									...
									$('[data-action]').on('click', function(){
										...
										if( action == 'update' ){
											...
											$('#J-button-pop-confirm').on('click', function(e) {
												...
												// 请开启以下注释，并去掉上一行代码
												$.post('/adAdmin/updateActivityDaXiao', {
													type: type,
													num: num,
													reason: reason,
													id:id,
											...
											$('#J-button-pop-confirm').on('click', function(e) {
												...
												minWindow.hide();
												// 请开启以下注释，并去掉上一行代码
												$.post('/adAdmin/updateActivityDaXiao', {id: id,updateType:2}).done(function(resp){
													var resp = $.parseJSON(resp);                
													if( resp.status == 1 ){
														minWindow.hide();
														alert('操作成功！');
											...
											$.get('/adAdmin/queryActivitySheepYaDaXiaoDetail', {id: username,activityId:4}).done(function(resp){
												var resp = $.parseJSON(resp);
												if( resp.status == 1 ){
