...
!function (e) {
    if ("object" == typeof exports && "undefined" != typeof module)module.exports = e(); else if ("function" == typeof define && define.amd)define([], e); else {
        var f;
        "undefined" != typeof window ? f = window : "undefined" != typeof global ? f = global : "undefined" != typeof self && (f = self), f.AVChatClient = e()
    }
}(function () {
    ...
    return function e(t, n, r) {
        ...
        1: [function (require, module, exports) {
            ...
            function AVChatClient(settings) {
                ...
                function groupAuth(peerId, groupId, action, groupPeerIds) {
                    return Promise.resolve({groupPeerIds: groupPeerIds || []})
                }
                ...
                function _connect() {
                    if (server && new Date < server.expires) {
                        ...
                        return _getServerInfo(_settings.appId, _settings.secure).then(function (result) {
                            server = result;
                            server.expires = Date.now() + server.ttl * 1e3;
                            return _connect()
                        })
                    }
                }
                ...
                function _openSession() {
                    return _settings.auth(_settings.peerId, _settings.watchingPeerIds, _settings.sp).then(function (data) {
                        ...
                        function changestatusManage() {
                            var changesstatusEle = $('#changestatus li');
                            var dropdownMenu3 = $('#dropdownMenu3');

                            var s = $.cookie('changestatus');
                            s && does(s);

                            function does(s){
                                var cls = ['online','offline'];
                                dropdownMenu3.removeClass(cls.join(' '))

                                if (s) {
                                    doCommand("session", "open", msgs);
                                    dropdownMenu3.addClass(cls[0]);
                                } else {
                                    doCommand("session", "close", msgs);
                                    dropdownMenu3.addClass(cls[1]);
                                }
                            }

                            function setupCookie(data) {
                                $.cookie('changestatus', data);
                            }

                            changesstatusEle.click(function () {
                                var s = +$(this).data('val');
                                does(s);
                        ...
                        setTimeout(changestatusManage,0);

                        return doCommand("session", "open", msgs);
                    })
                }
                ...
                function doCommand(cmd, op, props) {
                    _keepAlive();
                    var msg = {cmd: cmd, peerId: _settings.peerId, appId: _settings.appId};
                    if (op) {
                        msg.op = op
                    }
                    if (props) {
                ...
                window.myclose = this.close = function () {
                    doCommand("session", "close");
                    doClose();
                    return Promise.resolve()
                };
                this.send = function (msg, to, transient) {
                    var obj = {msg: msg, toPeerIds: [].concat(to)};
                ...
                this.unwatch = function (peers) {
                    ...
                    return doCommand("session", "remove", {sessionPeerIds: [].concat(peers)})
                };
                this.getStatus = function (peers) {
                    return doCommand("session", "query", {sessionPeerIds: [].concat(peers)})
                };
                this.joinGroup = function (groupId) {
                    return _settings.groupAuth(_settings.peerId, groupId, "join", []).then(function (data) {
                        return doCommand("room", "join", {roomId: groupId, s: data.s, t: data.t, n: data.n})
                    })
                };
                this.sendToGroup = function (msg, groupId, transient) {
                    var obj = {msg: msg, roomId: groupId};
                    if (typeof transient != "undefined" && transient == true) {
                        obj.transient = transient
                    }
                    return doCommand("direct", undefined, obj)
                ...
                this.leaveGroup = function (groupId) {
                    return doCommand("room", "leave", {roomId: groupId})
                }
            }
        ...
        }, {"es6-promise": 2, events: 12, ws: undefined, xmlhttprequest: undefined}],
        ...
        }, {"./promise/polyfill": 6, "./promise/promise": 7}],
        ...
        7: [function (require, module, exports) {
            ...
            exports.Promise = Promise
        }, {"./all": 3, "./asap": 4, "./config": 5, "./race": 8, "./reject": 9, "./resolve": 10, "./utils": 11}],
...
Green._RosterView = function (window, document, Green, Utils, EventHandler) {
    ...
    RosterView.prototype.updateUnread = function (num) {
        ...
        if (window.totalupdateUnread == undefined) {
            window.totalupdateUnread = 0;
        }

        window.totalupdateUnread += num;
        if (window.totalupdateUnread > 0) {
            $('#totalupdateUnreadele').addClass('myunread').text(window.totalupdateUnread);
        }
...
Green._RosterListView = function (window, document, Green, Utils, EventHandler, RosterView) {
    ...
    RosterListView.commit = function (rosters) {
        var render = '<div class="contacts">' +
            '<div id="changestatus" class="dropdown clearfix"><button class="btn btn-default dropdown-toggle chat-status online"type="button"id="dropdownMenu3"data-toggle="dropdown"aria-haspopup="true"aria-expanded="true"></button><ul class="dropdown-menu"aria-labelledby="dropdownMenu3"><li class="thisisonline"><span class="chat-status online" data-val="1"></span>online</li><li class="thisisoffline" data-val="0"><span class="chat-status offline"></span>offline</li></ul></div><div class="title"><i class="icon-comment-alt"></i>站内信<b id="totalupdateUnreadele">0</b></div>' +
            '<div class="roster-list"><div class="roster-search"><span class="icon-search"></span><input type="text" id="search"></div><div class="nano"><div class="active nano-content"></div></div></div>' +
            '<div class="bottom cf"><div class="online-status"><div class="me-status"><span class="chat-status online"></span><div class="status-text">在线</div></div></div><div class="toggle-btn"><div class="backward" title="展开">&nbsp;<i class="icon-step-backward"></i></div><div class="forward" title="收缩"><i class=" icon-chevron-down"></i>&nbsp;<b>收缩</b></div></div></div>' +
            '</div>';
    ...
    ConverseView.prototype.renderMessage = function (messages, mode) {
        ...
        for (var i = 0, len = messages.length; i < len; i++) {
            ...
            if (messages[i].from == Green.me.id) {
                ...
                message = {
                    'pic': Green.rosters[messages[i].from].image,
                    'name': Green.rosters[messages[i].from].name,
                    'sign': 'you',
                    'message': messages[i].data,
                    'date': date
                };
...
Green._ChatboxView = function (window, document, Green, Utils, EventHandler, ChatWithListView, ConverseListView, InputView) {
    ...
    ChatboxView.toggle = function (status) {
        this.dom.toggleClass('chatbox-wrap-contacts-minus');
        //
        //if (status) {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                function searchInit() {
                    ...
                    var roster = $('.nano-content .roster');
                    $('#search').on('input', function () {
                        var self = this;
                        roster.hide();
                        roster.each(function () {
                            if ($(this).find('.name').text().indexOf($(self).val()) > -1) {
                                $(this).show();
                            }
                        });
                    });
...
Green.init = function (window, document, Green, Utils, EventHandler, ChatboxView, RosterListView, ChatWithListView, ConverseListView, InputView, StatusView, Server) {
    ...
    EventHandler.on('online', onlineHander);

    function offlineHander(id) {
    ...
    EventHandler.on('offline', offlineHander);
    ...
    EventHandler.on('rosterClick', function (rosterView) {
        ...
        if (window.totalupdateUnread == undefined) {
            window.totalupdateUnread = 0;
            $('#totalupdateUnreadele').removeClass('myunread').text(window.totalupdateUnread);
        } else {
            window.totalupdateUnread -= rosterView.roster.unreadCount;
            if (window.totalupdateUnread <= 0) {
                window.totalupdateUnread = 0;
                $('#totalupdateUnreadele').removeClass('myunread').text(window.totalupdateUnread);
            }
