...
<head>
<title>查看下级报表</title>
<script type="text/javascript" src="${staticFileContextPath}/static/js/userCenter/subUserReport.js"></script>
...
<body>
	...
	<ul class="ui-search search-porxy clearfix" style="position:relative;">
		<li class="name"><label for="name" class="ui-label">用户名：</label>
			<input type="text" class="input" id="accountInput" value="${request.account}"	name="accountInput"  />
		...
		<li class="date"><label for="dateStar" class="ui-label">日期：</label>
			<input type="text" class="input" id="J-date-start" value="" name="queryTimeView">
			<input type="hidden" id="jdate" value="${request.queryTime}" name="queryTime" /></li>
		<li><a id="search" type="submit" class="btn btn-important"
			href="javascript:void(0);">搜 索<b class="btn-inner"></b></a></li>
	</ul>
</form>
				<div class="crumbs">
					<strong>客户层级：</strong><a href="${currentContextPath}/gameUserCenter/queryCurrentUserReport">报表查询</a> 
					<em id="userChainId">
					<c:forEach items="${chainUserProcess}" var="chainUser" varStatus="index">&gt;
					<a href="${currentContextPath}/gameUserCenter/querySubUserReport?userId=${chainUser.userId}&queryTime=${query.queryTime}&lotteryId=${query.lotteryId}&betTypeCode=${query.betTypeCode}">
					${chainUser.account}</a>
					</c:forEach>
					</em>
				</div>
  ...
  color:red
						...
						<tbody id = "tbd_dataEach">
							<c:forEach items="${cucrr.userCenterReportStrucs}" var="ucrs"
								...
								<tr>
									<td>${ucrs.account}</td>
									<td><c:if test="${ucrs.userLvl==0}">总代</c:if>
										<c:if test="${ucrs.userLvl==-1}">用户</c:if>
										<c:if test="${ucrs.userLvl>0}">代理</c:if></td>
									...
									<td><c:if test="${ucrs.userLvl!=-1 && ucrs.hasNextUser==1&&ucrs.userId!=query.userId}">
											<a id="subUserId_${ucrs.userId}"
												href="#">查看下级</a>
										</c:if>
										<c:if test="${ucrs.userLvl==-1|| ucrs.hasNextUser==0}">-</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					function addCommas(nStr) {
						...
						function ajaxGetMore() {
							...
							$.ajax({
								url:"${currentContextPath}/gameUserCenter/querySubUserReportAjax?userId=${query.userId}&queryTime=${query.queryTime}&lotteryId=${query.lotteryId}&betTypeCode=${query.betTypeCode}&pageNo="+$("#pageNo").val()+"&pageSize="+$("#pageSize").val()+"",
								dataType:'json',
								method:'post',
					...
					<form id="queryForm" action="${currentContextPath}/gameUserCenter/querySubUserReport?userId=${query.userId}&queryTime=${query.queryTime}&lotteryId=${query.lotteryId}" method="post">
					<!-- start  -->
					...
					<input id="pageSize" type="hidden" name="pageSize" value="${page.pageSize}">
					 ...
					 <div class="page-wrapper">
										<span class="page-text">
											<span class="lower">共${page.totalCount}条记录</span>
										</span>
...
<%-- 								<c:if test="${page.pageNo!=1}"><a href="#" onclick="doPre()" class="prev">上一步</a></c:if> --%>
								
<%-- 								<c:if test="${page.totalPages<=10}"> --%>
<%-- 									<c:forEach begin="1" end="${page.totalPages}" var="i"> --%>
<%-- 										<a href="#" onclick="doCurrent(${i})" <c:if test="${page.pageNo==i}">class="current"</c:if> >${i}</a> --%>
<%-- 									</c:forEach> --%>
<%-- 								</c:if> --%>
...
<%-- 								</c:if> --%>
<%-- 								<c:if test="${page.pageNo!=page.totalPages}"><a href="#" onclick="doNext()" class="next">下一步</a></c:if> --%>
<%-- 								<span class="page-few">到第 <input id="forwardPage" type="text" value="" class="input"> /${page.totalPages}页</span> --%>
