...
<script type="text/javascript"
	src="${staticFileContextPath}/static/js/userCenter/bettingRecords.js"></script>
<script type="text/javascript"
	src="${staticFileContextPath}/static/js/paging/paging.js"></script>
<script type="text/javascript">
		...
		<div class="common-article">
			...
			<div class="content">
				...
				<form action="queryOrders" method="post" id="J-form">
					<input type="hidden" id="J-data-now" value="" /> <input
						type="hidden" id="J-date-bound" value="" /> <input id="pageNo"
						type="hidden" name="pageNo" value="${page.pageNo }"> <input
					...
					<ul class="ui-search clearfix">
						...
						<li class="type"><label class="ui-label">彩种：</label> <input
							...
							<em id="sel"></em></li>
						<li class="time"><label class="ui-label">时间：</label> <span
							id="time_today" class="ico-tab " pro="1">今天</span> <span
							id="time_three" class="ico-tab " pro="3">三天</span> <span
							id="time_seven" class="ico-tab ico-tab-current" pro="7">七天</span>
							<input type="hidden" name="time" id="time" value="${time}" /></li>
						<li class="state"><label class="ui-label">状态：</label> <span
							class="ico-tab ico-tab-current" pro="0">全部</span> <span
							class="ico-tab" pro="2">已中奖</span> <span class="ico-tab" pro="3">未中奖</span>
							<span class="ico-tab" pro="1">等待开奖</span> <input type="hidden"
							name="status" id="status" value="${request.status}" /></li>
					</ul>
					<ul class="ui-search ui-search-more clearfix" id="divDetailed">
						<li class="issues"><label class="ui-label">期号：</label> <input
							type="hidden" id="webIssueCodeValue"
							value="${request.webIssueCode}"></input> <select
						...
						<li class="date"><label class="ui-label" for="dateStar">起始日期：</label>
						...
						<li class="btn-search"><a href="javascript:void(0);"
							class="btn btn-important" id="J-button-submit">确 定<b
								class="btn-inner"></b></a> <a href="javascript:void(0);"
							class="btn btn-link light" id="restoreDefaults">恢复默认项</a></li>
					</ul>
				</form>
					...
					<table class="table table-info">
						...
						<tbody>
							<c:forEach items="${orders}" var="orderStruc">
								<tr>
									<td>${orderStruc.orderCode}</td>
									<td>
									<c:forEach items="${orderId}" var="orderinfo">
										<c:if test="${orderStruc.orderId==orderinfo.orderId}">
											<a title="${orderinfo.playName}" href="#"  style = "text-decoration:underline;" >${orderStruc.lotteryName}</a>
										</c:if>
									</c:forEach>
									</td>
									<td>${orderStruc.webIssueCode}</td>
                                    ...
                                    <td><c:if test="${orderStruc.status!=2}"><c:if test="${orderStruc.status==7}">存在异常</c:if><c:if test="${orderStruc.status!=7}">${orderStruc.statusName}</c:if></c:if>
										<c:if test="${orderStruc.status==2}">
											<span class="price color-red"><dfn>&yen;</dfn><fmt:formatNumber 
													value="${orderStruc.totwin/10000.00}" pattern="#,###.##"  minFractionDigits="2" /></span>
										</c:if></td>

									<td><c:if test="${orderStruc.parentType==2}">是</c:if> <c:if
											test="${orderStruc.parentType!=2}">否</c:if></td>
									<td>${orderStruc.formatSaleTime}</td>
									<td><a
										href="${currentContextPath}/gameUserCenter/queryOrderDetail?orderId=${orderStruc.orderId}"
