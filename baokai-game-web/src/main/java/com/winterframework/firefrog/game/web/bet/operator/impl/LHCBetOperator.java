 ...
 * @Description 六合彩操作类
 */
public class LHCBetOperator extends AbstractBetOperator {
    private Logger logger = LoggerFactory.getLogger(LHCBetOperator.class);
    ...
    protected String formatLastBalls(GameIssueQueryResponse gameIssue) {
        return gameIssue.getNumberRecord() == null ? "01,02,03,04,05,06,07" : gameIssue.getNumberRecord().replaceAll("\\+",
                ",");
    }
    ...
    public List<List<NumberFrequencyCell>> frequency(@RequestParam("gameMode") String gameMode,
        ...
        if ("lost".equals(frequencyType)) {
            // 遗漏
        	logger.info("query frequency:makeGameMissingValueQueryRequest start");
            GameMissingValueQueryRequest request = makeGameMissingValueQueryRequest(gameMode, extent);
            logger.info("query frequency:makeGameMissingValueQueryRequest End");
            try {
            	logger.info("查询遗漏開始");
