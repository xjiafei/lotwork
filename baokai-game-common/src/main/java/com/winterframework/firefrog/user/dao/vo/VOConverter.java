...
public class VOConverter {
	...
	public static User customer2User(UserCustomer customer) throws Exception {
		...
		if (customer.getUserLvl() != null && customer.getUserLvl() >= 0) {
			...
			if (null != customer.getAgentLimit()) {
				useragent.setAgentLimit(customer.getAgentLimit().intValue());
			}
			...
			user = new User();
			user.setUserLevel( customer.getUserLvl());
		}
		...
		user.setVipLvl(customer.getVipLvl().intValue());
		...
		user.setHeadImg(customer.getHeadImg());
		...
		// profile.setQq(((UserProfile)DataConverterUtil.convertJson2Object(customer.getQqStruc(),
		// UserProfile.class)).getQq());
		// profile.setQa(((UserProfile)DataConverterUtil.convertJson2Object(customer.getQuStruc(),
		// UserProfile.class)).getQa());
		try {
	...
	public static UserCustomer convertUser2Customer(User user) {
		...
		if(user.getHeadImg() != null){
			customer.setHeadImg(user.getHeadImg());
		}
		...
		if (userProfile != null) {
			...
			customer.setSource(userProfile.getSource());
		}
...
//				String[] temp = um.getReceivers().split(",");
//				for (String s : temp) {
//					receivers.add(getUser(0L, s.trim()));
//				}
//			}
//		}
...
//			} else {
//				String[] ids = new String[mt.getReceiver().size()];
//				int i = 0;
//				for (User u : mt.getReceiver()) {
//					ids[i] = u.getUserProfile().getAccount();
//					i++;
//				}
//				String recStr = Arrays.toString(ids);
//				um.setReceivers(recStr.substring(1, recStr.length() - 1));
//			}
//		}
//
...
//		appeal.setPassed(_appeal.getPassed().intValue());
...
//	}
	...
	public static UserCenterReportInfo userCenterReportVo2UserCenterReportInfo(UserCenterReportVo gi) {
		UserCenterReportInfo ui =new UserCenterReportInfo();
		User user=new User();
		user.setId(gi.getUserId());
		user.setUserLevel(gi.getUserLvl());
		UserProfile up=new UserProfile();
		up.setAccount(gi.getAccount());
		up.setUserChain(gi.getUserChain());
		user.setUserProfile(up);
		ui.setUser(user);
		ui.setTotalSubuserPoint(gi.getTotalSubuserPoint());
		ui.setTotalSubuserSaleroom(gi.getTotalSubuserSaleroom());
        ui.setActualTotalSubuserSaleroom(gi.getTotalSubuserSaleroom()-gi.getTotalSubuserPoint());
        ui.setTotalSubuserWins(gi.getTotalSubuserWins());
        ui.setActivityGifts(gi.getActivityGifts());
        ui.setTotalProfit(ui.getActualTotalSubuserSaleroom()-ui.getTotalSubuserWins());
        ui.setHasNextUser(gi.getHasNextUser());
		return ui;
	}
