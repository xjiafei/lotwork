...
@Controller("slmmcController")
@RequestMapping("/slmmc")
public class SLMMCController extends BaseController {

	private static final Logger log = LoggerFactory
			.getLogger(SLMMCController.class);
	...
	private static final Long LOTTERYID = 99112L;

	private static final String LOTTERYCODE = "slmmc";
...
//			ut.setUserId(1300271L);
			...
			log.error("slmmc initData error : ", e);
	...
	public Response<GameBetJsonResultStruc> betSubmit(
		...
		String token = request.getHead().getSessionId();
		log.info("----------------------mobile slmmc bet start!!!");
		...
		try {
			for (GameSlip slip : gameOrder.getSlipList()) {
				IKeyGenerator keyGen = new LotteryPlayMethodKeyGenerator(
						99112L, slip.getGameBetType().getGameGroupCode(), slip
				...
				IValidateExecutor<GameSlip> validatorExecutor = validatorFactory
						.getValidateExecutor("slmmc", keyGen);
