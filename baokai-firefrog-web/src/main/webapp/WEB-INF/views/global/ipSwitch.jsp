...
<c:when test="${fn:contains(aclList,'GLOBAL_IPILLEGALITY') || fn:contains(aclList,'GLOBAL_IPLEGAL')}">
			...
			<div class="col-crumbs"><div class="crumbs"> <strong>当前位置：</strong><a href="#">全局管理</a> &gt; IP黑名单 </div></div>
					...
					<div class="main">
						...
						<div class="ui-tab">
							<div class="ui-tab-title clearfix">
								<ul>
									<permission:hasRight moduleName="GLOBAL_IPILLEGALITY"><li class="current">黑名单设置</li></permission:hasRight>
									<permission:hasRight moduleName="GLOBAL_IPLEGAL"><li><a href="${currentContextPath}/globeAdmin/goIpSwitch?type=1">白名单设置</a></li></permission:hasRight>
								</ul>
							</div>
							<div class="ui-tab-content ui-tab-content-current">
								...
								<div style="padding:10px 0 0 10px;">
									...
									<div id="J-switch" class="switch has-switch">
										<div <c:if test="${bStatus==1}">class="switch-on"</c:if><c:if test="${bStatus==0}">class="switch-off"</c:if>>
											<span class="switch-left">开启</span><label>&nbsp;</label><span class="switch-right">关闭</span>
											<input class="switch-type" type="hidden" value="0" />
											<input class="switch-value" type="hidden" value="${bStatus}" />
										</div>
									</div>
								</div>
								...
								<ul class="ui-search">
									...
									<li>
										...
										<input id="J-days" name="period" type="text" class="input w-1"  value="" maxlength="5" style="text-align:center;">
								...
								<div class="page-wrapper clearfix" style="float:left;">
									<span class="page-text">
										...
										<permission:hasRight moduleName="GLOBAL_IPILLEGALITY">
...
function doPre(){
	...
	location.href = baseUrl+'/globeAdmin/goIpSwitch?pageNo='+Number(parseInt(currentPageNo)-1);
}
...
function doForward(index){
    ...
    location.href = baseUrl+'/globeAdmin/goIpSwitch?pageNo='+$("#pageNo").val();
}

function doCurrent(pageNo){
	$("#pageNo").val(pageNo);
	location.href = baseUrl+'/globeAdmin/goIpSwitch?pageNo='+pageNo;
}
</script>	
</c:when>
<c:otherwise>
