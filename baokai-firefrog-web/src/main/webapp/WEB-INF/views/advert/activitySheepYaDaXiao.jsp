							...
							<div class="ui-tab-content ui-tab-content-current">
								...
								<ul class="ui-form">
									<li>
										...
										<span class="ico-tab ico-tab-item ico-tab-current"><a href="${currentContextPath}/adAdmin/queryActivityYaDaXiaoLog">全部</a></span>
										...
										<span class="ico-tab ico-tab-item"><a href="${currentContextPath}/adAdmin/queryActivityYaDaXiaoLog?pageStatus=5">未通过</a></span>
									</li>
								</ul>
								
								<form action="queryActivityYaDaXiaoLog" id="J-form" method="post">
									...
									<ul class="ui-search">
										...
										<li>
											...
											<input id="updateStatusv" type="hidden" value="${query.updateStatus}">
										</li>
										<li>
											<a id="J-button-submit" class="btn btn-important" href="javascript:void(0);">搜 索<b class="btn-inner"></b></a>
											<a id="J-download-submit" class="btn btn-important" href="${fromGame}/gameoa/exportActivityYaDaXiaoLog">导出报表<b class="btn-inner"></b></a>
										</li>
									</ul>
								</form>
								...
								<table class="table table-info">
									...
									<c:forEach items="${struc}" var="struc" varStatus="configIndex">
										<tr data-id="${struc.id}">
											...
											<span data-type="increase" data-updatetimes="${struc.updateLastNum }" data-last="${struc.lastNum+struc.updateLastNum}" <c:if test="${struc.updateStatus==1}">data-showtip="${struc.lastNum}"</c:if> data-times="${struc.lastNum }">${struc.lastNum+struc.updateLastNum}</span></td>
											<td>${struc.nextWinNumNow}</td>
											<td>${struc.maxNextWinNum}</td>
											...
											<td>
												...
												<permission:hasRight moduleName="MARKET_ACTIVITY_UPDATEACTIVITYYADAXIAO">
												<c:if test="${struc.updateStatus==0 || struc.updateStatus==null}"><a data-action="update" href="javascript:void(0);">更新</a></c:if></permission:hasRight>
												<permission:hasRight moduleName="MARKET_ACTIVITY_PUBLISHACTIVITYYADAXIAO">
								...
								<script type="text/javascript" src="${staticFileContextPath}/static/js/advert/activitySheepYaDaXiao.js"></script>
								<!-- PAGER 分页 -->
