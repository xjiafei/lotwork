...
<body>		<permission:hasRight moduleName="MARKET">
			<div class="col-crumbs">
                <div class="crumbs">
			<permission:hasRight moduleName="MARKET_ADDADVERT">
				<a href="${currentContextPath }/adAdmin/toAddAdvert" class="more">新建广告</a>
			 </permission:hasRight>
			 <strong>当前位置：</strong>
                    <a href="${currentContextPath}/adAdmin/queryPublishAdPage">营销中心</a> &gt; 广告管理（发布）
                </div>
            </div>
			...
			<div class="col-content">
				<div class="col-main">
					<div class="main">
						<form action="${currentContextPath }/adAdmin/queryPublishAdPage" id="J-form" method="post">
						<ul class="ui-search">
							<li>
						...
						<div class="ui-tab">
							<div class="ui-tab-title clearfix">
								<ul>
									...
									<c:if test="${adQueryRequest.status == 4 }">
									<li class="current"><a href="javascript:void(0);" id="4">待审核(${sumWait })</a></li>
									</c:if>
									<c:if test="${adQueryRequest.status != 4 }">
									<li><a href="javascript:void(0);" id="4">待审核(${sumWait })</a></li>
									</c:if>
									...
									<c:if test="${adQueryRequest.status == 5 }">
									<li class="current"><a href="javascript:void(0);" id="5">已删除</a></li>
									</c:if>
									<c:if test="${adQueryRequest.status != 5 }">
									<li><a href="javascript:void(0);" id="5">已删除</a></li>
									</c:if>
								</ul>
							</div>
							<div class="ui-tab-content ui-tab-content-current">
								<table class="table table-info table-group" id="J-table">
									...
									<tbody>
										<c:forEach items="${adStrucs }" var="adStruc" varStatus="status">
											...
											<c:if test="${adStruc.status != 1 && adStruc.status != 5}">				
											...
											<td><a href="${adStruc.targetUrl }"　target='_blank'>${adStruc.targetUrl }</a></td>
											...
											<td class="table-tool">
												<c:if test="${adStruc.status == 4 || adStruc.status == 2 || adStruc.status == 3}">
												<a class="ico-edit" title="编辑" href="${currentContextPath }/adAdmin/toUpdateAdvert?id=${adStruc.id }"></a>
												
												<a data-id="${adStruc.id }" class="ico-delete row-delete" title="删除" href="javascript:void(0);"></a>
								...
								<div class="page-wrapper" style="float:left">
									<c:if test="${adQueryRequest.status == null || adQueryRequest.status==2 || adQueryRequest.status==3}">
									...
									<c:if test="${adQueryRequest.status == 4 }">
										<span class="page-text">
											<label class="label"><input id="J-select-all" type="checkbox" class="checkbox">全选</label>
											<a id="J-button-passall" class="btn btn-small" href="javascript:void(0);">删除<b class="btn-inner"></b></a>
											<a id="J-button-submit" class="btn btn-small" href="javascript:void(0);">提交审核<b class="btn-inner"></b></a>
										</span>
									</c:if>
								</div>
			...
			<script type="text/javascript" src="${staticFileContextPath}/static/js/advert/advertPublishManage.js"></script>
			</permission:hasRight>
</body>
</html>