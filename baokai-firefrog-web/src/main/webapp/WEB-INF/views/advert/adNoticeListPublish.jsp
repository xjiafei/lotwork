...
<body>
			<div class="col-crumbs">
                <div class="crumbs">
                    <strong>当前位置：</strong>
                    <a href="${currentContextPath}/adAdmin/queryPublishAdPage">营销中心</a> &gt; 公告管理（发布）
                </div>
            </div>
						...
						<div class="ui-tab">
							<div class="ui-tab-title clearfix">
								<ul>
									...
									<li <c:if test="${search.status==1}"> class="current"</c:if>><a href="javascript:changeStatus(1,'GMT_CREATED DESC')">待审核（${sumNew }）</a></li>
									...
									<li <c:if test="${search.status==5}"> class="current"</c:if>><a href="javascript:changeStatus(5,'GMT_MODIFIED DESC')">已删除</a></li>
								</ul>
							</div>
							...
							<c:if test="${!empty list}">							
								<table class="table table-info table-group" id="J-table">
									...
									<c:forEach items="${list}" var="list" varStatus="status">
										<tr>
											<td>
												<label class="label">
												<c:if test="${list.status!=5 && list.status!=2}">
											...
											<c:if test="${list.status!=5}">
												...
												<c:if test="${list.status!=2}">
													<a class="ico-edit" title="编辑" href="${currentContextPath}/adAdmin/goModifyNotice?id=${list.id}"></a>
													<a data-id="${list.id}" class="ico-delete row-delete" title="删除" href="javascript:void(0);"></a>
												</c:if>
											</c:if>
											</td>
										</tr>
								...
								<div class="page-wrapper clearfix" style="float:left;">
									<span class="page-text">
										...
										<c:if test="${search.status!=5&&search.status!=2}">
											<a id="J-button-passall" class="btn btn-small" href="javascript:void(0);">删除<b class="btn-inner"></b></a>
										</c:if>
										<c:if test="${search.status==1}">
											<a id="J-button-submitall" class="btn btn-small" href="javascript:void(0);">提交审核<b class="btn-inner"></b></a>
										</c:if>
									</span>
								</div>
...
<script type="text/javascript" src="${staticFileContextPath}/static/js/advert/adNoticeListPublish.js"></script>			
</body>