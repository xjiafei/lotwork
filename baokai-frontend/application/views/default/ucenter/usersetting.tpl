							...
							<div class="rebate-list">
								<ul class="rebate-list-setup rebate-list-ico">
									<li>
										...
										<span class="ico-tab" id="userAgent">代理</span>
										<span class="ico-tab ico-tab-current" id="userPlayer">玩家</span>
										<input type="hidden" id="hiddenType" name="type" value="0"/>
									</li>
									<li>
									...
									<li>
										...
										<input type="hidden" maxlength="16" class="input w-3" id="Remarks" value="111111"/>
									</li>
								...
								<li>
                                    ...
                                    <li id="li-QuickSetup" style="display:none">
                                        ...
                                        <!--<input type="input" class="input" id="Rebate" name="status_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}" onKeyUp="CheckingMaxAllFull(this,4.9);" onBlur="CheckingMaxAllFull(this,4.9);" />-->
										<input type="input" class="input" id="Rebate" name="Rebate" onKeyUp="CheckingMaxAllFull(this,4.9);" onBlur="CheckingMaxAllFull(this,4.9);" />
											...
											<dd>
												...
												<!-- {if $sub_data.lotteryId !='99701'} 六合彩-->
													<tr>
														...
														<!-- {/if} -->
														<!--<td rowspan="2"><input type="checkbox" id="check" class="checkbox" name="status_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}" value="{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}_{$sub_data.awardGroupId}"/></td>-->
												...
												<!-- {/if} -->
													...
													<tr>
														...
														<tr><td></td>														
															<td style="text-align:left;">
																<span class="label-like">{$aRateName[$sub_data.lotterySeriesCode][1]}</span>
																<input type="input" class="input w-1 ldc1" name="directRet_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}_{$sub_data.awardGroupId}"  value="" maxvalue="{$directLimitRet}" onKeyUp="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$directLimitRet}')" onBlur="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$directLimitRet}')"/>
														...
														<tr><td></td>
															<td style="text-align:left;">
																...
																<input type="input" class="input w-1 normal"  
																onKeyUp="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$lhcYearLimit}')" onBlur="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$lhcYearLimit}')" class="input w-1"  maxvalue="{$lhcYearLimit}" value="" name="lhcYear_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}_{$sub_data.awardGroupId}"/>
														...
														<tr><td></td>
															<td style="text-align:left;">
																...
																<input type="input" class="input w-1 normal" 
																onKeyUp="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$lhcColorLimit}')" onBlur="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$lhcColorLimit}')" class="input w-1"  maxvalue="{$lhcColorLimit}" value="" name="lhcColor_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}_{$sub_data.awardGroupId}"/>
													...
													<tr>
														<td style="text-align:left;">
															<span class="label-like">{$aRateName[$sub_data.lotterySeriesCode][2]}</span>
															<input type="input" class="input w-1 2000" onKeyUp="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$superLimitRet}')" onBlur="calcu_prize(this,'{$award}','{$theoryAward}');CheckingMaxAllFull(this,'{$superLimitRet}')" class="input w-1"  maxvalue="{$superLimitRet}" value="" name="superRet_{$sub_data.lotterySeriesCode}_{$sub_data.lotteryId}_{$sub_data.awardGroupId}" />
...
<div class="pop w-7" style="position:absolute;left:500px;display:none" id="divRemarksIsEmpty">
	<div class="hd"><i class="close" name="divCloseUrl"></i><span>提示</span></div>
	<div class="bd">
		<h4 class="pop-title">链接备注不能为空，请重新填写</h4>
		<div class="pop-btn">
			<a href="javascript:void(0);" class="btn btn-important " name="J-but-close">确 定<b class="btn-inner"></b></a>
		</div>
	</div>
</div>
<div class="pop w-7" style="position:absolute;left:500px;display:none" id="divRemarksIsLength">
	<div class="hd"><i class="close" name="divCloseUrl"></i><span>提示</span></div>
	<div class="bd">
		<h4 class="pop-title">链接备注的字符长度为4-16字符</h4>
		<div class="pop-btn">
			<a href="javascript:void(0);" class="btn btn-important " name="J-but-close">确 定<b class="btn-inner"></b></a>
		</div>
	</div>
...
<div class="rate-info-jump">一键设置您和直属下级的<em>返点差额</em>，输入值高于您拥有的返点时下级返点清零。<br>（设置后可能需要手动调整的有：时时彩-所有超级2000玩法，低频-六合彩）</div>
<!-- //////////////底侧公共页面////////////// -->
...
function check_qq()
{
  ...
  (function($){	
	...
	var inputs = $('.table').find('[type="input"]'),addfile,box = new LightBox("divIsEmpty"),box2=new LightBox("divRegSuccess"),box3=new LightBox("divRemarksIsEmpty"),box4=new LightBox("divNoType"),box5=new LightBox("divRemarksIsLength");	
	if(emptyRewards ==1){
...
$("#returnZero").css("border","none");
		...
		/*if(hiddenTypes == '1'){			
			...
			$('.item').each(function() {				
				$(this).find('dd:first').find('.table').find('.radio').attr("checked", "checked");
			});				
		}*/
...
$("#returnAll").css("border","none");
    ...
    $("#span-QuickSetup").click(function(){
		...
		$("#li-QuickSetup").css("display","inline");
	...
	$("#userAgent").click(function(){
		window.location="/applycenter/index?type=1";
	});
	...
	$('#J-btn-Subm').click(function (e){	
		...
		if($.trim($("#Remarks").val())=="")
		{
			...
			box3.Center = true; 	
			box3.Show();
			$("#Remarks").css("border", "solid 1px red")
			$("#Remarks").focus();
			istrue=false;
			return false;
		}
		if(WidthCheck($.trim($("#Remarks").val()))< 4 || WidthCheck($.trim($("#Remarks").val()))>16){
			fn("divRemarksIsLength");
			box5.Over = true;
			box5.OverLay.Color = "rgb(0, 0, 0)" ;
		...
		if(istrue==true){
			...
			var shidden=GetPostText("div-context")+"type="+$("#hiddenType").val()+"&Remarks="+''+"&days="+$("[name='days']").val()+"&qq="+$("#hiddenqq").val()+"&SetUp="+$("#hiddenSetUp").val();
			$.ajax({
				type:'post',
				dataType:'json',
				cache:false,
				...
				success:function(data){				
					if(data['status'] == 'ok'){						
						...
						$('#divCloseUrl').click(function(){
							$('#divRegSuccess').css("display","none");
						});
					}
					else{
						//失败提示后隐藏
