			...
			<div class="common-article">
				<div class="title">
					<div class="more">
						...
						<a href="/Service/servicesup?unread={$unread}" class="current">给上级发信</a>
						{/if}
...
<script >     
  (function($){	
	  ...
	  $('#subSend').click(function(e){		  	
		...
		var Ttitle=$('#Txttitle').val(),Ttitle = Ttitle.replace(/[^\x00-\xff]/g, 'xx'),TxtText=$('#TxtText').val(),TxtText = TxtText.replace(/[^\x00-\xff]/g, 'xx'),istrue=true;	
	    if(Ttitle=='' ||  Ttitle.length>30)  { TxttitlePar.find('.ui-check').css('display', 'inline'); TxttitlePar.find('.ui-text-prompt').css('display', 'none');  e.preventDefault();   istrue=false;  }	
		...
		$.ajax({
				...
				data:'title='+$("#Txttitle").val().trim()+'&content='+$("#TxtText").val().trim()+'&typeid='+2,	
				beforeSend:function(){
					//禁用发送												
				...
				success:function(json){	
					if(json !=null){
						if(json['status']=="0"){	fn("DivDeleteok");	window.location = "/Service/inbox";	}
