...
<head>
    ...
    <script type="text/javascript" src="{$path_img}/js/phoenix.Common.js"></script>
	...
	<div class="g_33 common-main">
		...
		<div class="g_28 g_last">
			<div class="common-article">
				...
				<div class="content">
					<div class="ui-tab">
						...
						<div class="">
							<!-- start -->
							<form action="/withdraw/history" method="post">
							...
							<table class="table table-info">
								...
								<!-- {foreach from=$content item=data} -->
									<tr>
										<th {if $data.seperateCount neq 1}class="sp-td"  onclick="showTip('{$data.tipData}');"{/if}>{$data.sn}</th>
										<th>{$data.applyTime}</th>
										<th>{$data.withdrawAmt}</th>
										<th>{$data.realWithDrawAmt}</th>
										<th>{$data.bankname}</th>
										<th>{$data.banknumber}</th>
										<th>{$data.status}</th>
										<th><a href='javascript:void(0);' name='queryProcessPage' style='position:initial'>查看</a></th>						
									</tr>
								<!-- {/foreach} -->
								</tbody>
							</table>
							...
							<!-- {/if} -->
							<div class="cumulative-recharge">累计提现：{$withdrawAmt}元</div>
							<!-- {if $pages} -->
			...
			<dl class="fund-info-supplement">
				<dt>说明：您可以查询您的账户最近7天内的提现记录。</dt>
			</dl>
		</div>
        <!--新增审核内容display:none;-->
        <div class="pop" id="tipPopup" style="position:fixed; top:50%; left:50%;width:auto;height:auto;opacity: 0.8;display:none;">
...
(function(){
	...
	//查看
	$(document).on('click', '[name="queryProcessPage"]', function(e){
		var oTr=$(this).parent().parent();		
		var sdata="sn="+$.trim(oTr.find("th:eq(0)").text());
		window.open("/withdraw/queryprocess"+"?"+sdata);	
	});
