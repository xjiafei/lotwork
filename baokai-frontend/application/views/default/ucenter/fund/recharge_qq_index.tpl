						...
						<table class="step-table">
							<tbody>
								<tr>
									...
									<td {if $stp eq 2}class="current"{/if}><div class="tri"><div class="con">3.登陆手机QQ付款</div></div></td>
								</tr>
							</tbody>
						</table>
					</div>
...
(function(){
	...
	$('#J-next-step').click(function(){
		...
		$.ajax({
			...
			dataType:'json',
			cache:false,
			data:"chargeamount="+$('#chargeamount')[0].value+"&bankid=35",
			success:function(data){
				if(data['chargeLimit']==0){
					minWindow.setTitle('温馨提示');
					minWindow.setContent('<div class="pop-btn"><i class="ico-waring <#=icon-class#>"></i><h4 class="pop-text">QQ钱包充值已达到单日上限！</h4></div><div class="pop-btn">请您明日0点再次尝试，建议先使用其他充值方式充值</div><div class="pop-btn"><a class="btn closeBtn" href="#">关 闭<b class="btn-inner"></b></a></div>');
					minWindow.show();;
