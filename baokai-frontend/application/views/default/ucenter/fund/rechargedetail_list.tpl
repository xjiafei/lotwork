	...
	<div class="g_33 common-main">
		...
		<div class="g_28 g_last">
			<div class="common-article">
				<div class="notice"><i class="ico-warning"></i>由于需要与银行数据对接，汇款成功后系统会在十五分钟内处理您的申请。如有问题，请 <a  href="http://v88.live800.com/live800/chatClient/chatbox.jsp?companyID=740552&configID=2945&jid=4611765122&skillId=145" target="_blank">联系客服</a></div>
						...
						<div class="">
							...
							<ul class="ui-search search-fund clearfix">
								...
								<li class="number">
									...
									<input type="text" value="{$_POST.sn}" name="sn" class="input">
								</li>
								<li class="date">
								...
								<li class="status">
									...
									<select class="ui-select" name="type">
										<!-- {foreach from=$aTypeArray item=data key=key} -->
											<!-- {if $key neq '4'} -->
												<option value="{$key}" {if $_POST.type eq $key}selected{/if}>{$data}</option>
											<!-- {/if} -->
										<!-- {/foreach} -->
									</select>
								</li>
							...
							<table class="table table-info">
								...
								<!-- {foreach from=$content item=data} -->
									<tr>
										...
										<td>{if $data.depositMode neq ''}{$data.depositMode} [{$data.bankname}]{else}{$data.bankname}{/if}</td>
										<td>{$data.status}</td>
									</tr>
								<!-- {/foreach} -->
								</tbody>
							...
							<!-- {/if} -->
							<div class="cumulative-recharge">累计充值：{$applyAmt}元</div>
							<!-- {if $pages} -->
								<div class="page-wrapper">
									<span class="page-text">共{$total}条记录</span>
									<div class="page page-right">
										<!-- {if $pages.pre && $pages.currpage.index!=1} -->
			    							<a  class="prev" onClick="$('#fm_main').submit();" href="/fund/history?page={$pages.pre.index}&sn={$_POST.sn}&fromDate={$_POST.fromDate}&toDate={$_POST.toDate}&status={$_POST.status}&type={$_POST.type}">上一页</a>
			    						<!-- {/if} -->
				    					<!-- {foreach from=$pages.steps item=item} -->
				    						<!-- {if $item.index == $pages.currpage.index} -->
				    							<a class="current" href="javascript:void(0);">{$item.text}</a>
				    						<!-- {else} -->
				    							<a onClick="$('#fm_main').submit();" href="/fund/history?page={$item.index}&sn={$_POST.sn}&fromDate={$_POST.fromDate}&toDate={$_POST.toDate}&status={$_POST.status}&type={$_POST.type}">{$item.text}</a>
				    						<!-- {/if} -->
				    					<!-- {/foreach} -->
				    					<!-- {if $pages.next && $pages.currpage.index!=$pages.max.index} -->
				    						<a  class="next" onClick="$('#fm_main').submit();" href="/fund/history?page={$pages.next.index}&sn={$_POST.sn}&fromDate={$_POST.fromDate}&toDate={$_POST.toDate}&status={$_POST.status}&type={$_POST.type}">下一页</a>
				    					<!-- {/if} -->
