	...
	<div class="col-content">
			...
			<!-- /////////////头部公共页面/////////////// -->
		<div class="col-main">
			<div class="col-crumbs"><div class="crumbs"><strong>当前位置：</strong><a href="#">资金</a> &gt; <a href="#">客户提取</a> &gt; 客服提现</div></div>
			<div class="col-content">
				<div class="col-main">
					<div class="main">
						<div class="ui-tab">
							<div class="ui-tab-title clearfix">
								<ul>
									<li class="current"><a href="/admin/Rechargemange/customersdraw?swithval=1">客户提现</a></li>
									...
									<li><a href="/admin/Rechargemange/customersdraw?swithval=5">付款失败</a></li>
								</ul>
							</div>
...
(function() {
					...
					$.each(re, function (i) {						
						...
						switch(re[i].status)
						{
							case "0":
								...
								html += "<td>/</td>";
								html += "<td class='sele_"+i+"'><select name='sele_status' pro= "+re[i].sn +"  onchange='Updatestatus(this)' id='sele_"+i+"'  pro_s='stu_"+i+"' ><option value='3' selected='true'>--选择操作--</option><option value='1'>通过</option><option value='0'>拒绝</option></select></td>";
							...
							case "1":
								...
								break;	
							case "2":
								html += "<td><span >等待付款</span></td>";
								html += "<td>/</td>";
								html += "<td></td>";
								html += "<td>"+objString+"</td>";
								break;	
							case "3":
								html += "<td><span >提现成功</span></td>";
	...
	$('#chargeUpdate').click(function(e){		
		if($('#TxtText').val()!=""){
			...
			var exceptId = mowId,mask = phoenix.Mask.getInstance();
			
			$.ajax({
				url:'/admin/Rechargemange/withdrawremark',
				cache:false,
				dataType:'json',
				method:'post',
	...
	function Updatestatus(obj){			
		...
		var sele_status = $("select option:selected").val();	//当前控件id
		if((upid!=''&&updatid!='') || upid!='0')
		{
			...
			$.ajax({
				url:'/admin/Rechargemange/withdraw',
				cache:false,
				dataType:'json',
				method:'post',
				data:'mowId='+updatid+'&opterID='+upid+'&status='+sele_status,
				beforeSend:function(){
					mask.dom.addClass('admin-mask-loading');
					mask.css({opacity:.9,backgroundColor:'#FFF'});
					mask.show(dataArea);
				...
				success:function(data){
					if(data.status=="1"){
						...
						if(upid=='1'){
							...
							$('#'+ups).html("<span style='color:red'>提款失败</span>");
