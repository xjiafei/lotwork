...
<!-- /////////////头部公共页面/////////////// -->
	<div class="col-main">
			<div class="col-crumbs"><div class="crumbs"><strong>当前位置：</strong><a href="#">资金</a> &gt; <a href="#">异常充值</a> &gt;  <a href="#">异常充值</a> &gt;异常处理</div></div>
			<div class="col-content">
				<div class="col-main">
					<div class="main">
						<div class="ui-tab">
							<div class="ui-tab-title clearfix">
								<ul>
                                    <li class="current"><a href="/admin/Rechargemange/exceptrecharge?swithval=1">异常处理</a></li>
									<li ><a href="/admin/Rechargemange/exceptrecharge?swithval=2">等待审核</a></li>
                                           ...
                                           <th  id="J-sp-serial" class="sp-td">
											...
											<th id="J-sp-request-Audit" class="sp-td">
                                                <div class="sp-td-cont sp-td-cont-b">
                                                    ...
                                                    <div class="sp-filter-cont sp-filter-cont-b sp-filter-cont-select" style="">
                                                        <ul>
                                                            <li data-select-id="1"><a href="#">通过</a></li>
...
(function() {
	...
	function SelectByWhereInfo(pages) {
		...
		var perPageNum  = new phoenix.SuperSearch({
		 ...
		 $.ajax({
			url:'/admin/Rechargemange/exceptrechagedl?swithval=1',
			dataType:'json',
			method:'post',
			data:formData,
			beforeSend:function(){
			...
			success:function(data){
				...
				if(data.text){
					...
					$.each(re, function (i) {						
						...
						switch(re[i].status)
						{
							case "0":
								...
								html += "<td class='sele_"+i+"'><select name='sele_status' onchange='Updatestatus(this)' id='sele_"+i+"' pro='"+re[i].mcSn+"' pro_s='stu_"+i+"' ><option value='0' selected='true'>--选择操作--</option><option value='1'>加币成功</option><option value='2'>退款成功</option><option value='3'>没&nbsp;&nbsp;收</option></select></td>";
								break;							
							case "1":
								html += "<td>加币成功</td>";
								html += "<td>成功</td>";
							...
							case "8":
								html += "<td>退款失败</td>";
								html += "<td><span style='color:red'>退款失败</span></td>";
								break;
							case "9":
								html += "<td>加游戏币审核通过</td>";
								html += "<td>成功</td>";
					...
					$("#J-table-data>tbody").html(html);
					 ...
					 $("#Pagination").hide();                 
                    $("#J-table-data>tbody").html("<tr><td colspan=\"15\">没有相应数据</td></tr>");
				}
...
//备注层展示(获取参数)
	...
	function Updatestatus(obj){	
		...
		if((upid!=''&&updatid!='') || upid!='0')
		{
			...
			$.ajax({
				url:'/admin/Rechargemange/dealway',
				dataType:'json',
				method:'post',
