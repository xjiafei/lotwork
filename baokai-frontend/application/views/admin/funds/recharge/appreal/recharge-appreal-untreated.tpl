...
<div class="col-content">
    ...
    {literal}
        <script>
             ///admin/Rechargemange/index?parma=review&type=aprluntreated&appealSn=" + re[i].appealSn  + "'                    
             function review(sn){
                 console.log(sn);
                 $.ajax({
                     url:'/admin/Rechargemange/index?parma=chr&appealSn=' + sn,
                     method:'post',
                     success:function(data){
                        if(data.length > 0){
                            var result = JSON.parse(data);
                            if(result.isCheck==1){
                                $("#"+sn).text(result.checkUser + "审核中...");
                            }else{
                                window.location = "/admin/Rechargemange/index?parma=review&type=aprluntreated&appealSn=" + sn;
                            }
                        }
                     }
            ...
            (function () {
                ...
                function UntreatedData() {
                    ...
                    function SelectByWhereInfo2(pages) {
                        ...
                        $("#Pagination2").hide();
                        $.ajax({
                            url: '/admin/Rechargemange/index?parma=aprl&status=1',
                            dataType: 'json',
                            method: 'post',
                            data: formData2,
                            beforeSend: function () {
                            ...
                            success: function (data) {
                                if (data.text.length > 0) {
                                    ...
                                    $.each(re, function (i) {
                                        ...
                                        if(resultAll.auditPermission == 'Y'){
                                            html += "<td id=" + re[i].appealSn + "><a class='btn btn-small' style='position:initial' href='#' onclick=review('" + re[i].appealSn + "') ><font color='#25a38a'>审 核</font><b class='btn-inner'></b></a>&nbsp;</td>";
                                        }else{
                                            html += "<td id=" + re[i].appealSn + ">&nbsp;</td>";
                                        }
