                ...
                <div class="main">
                    <div class="ui-tab">
                        <ul class="ui-tab-title ui-tab-title2 clearfix" id="Tabs">
                            ...
                            <span><a href="/admin/Rechargemange/index?parma=abcf"><li class='current'>申诉银行配置</li></a></span>
                            <!--{else}-->
                            <span><li style="display:none;"></li></span>
                            <!-- {/if} -->
                            <!-- {if "FUND_APPEAL_BANKS_CONFIG"|in_array:$smarty.session.datas.info.acls} -->
                        ...
                        <ul class="ui-form"  id="DivRules">
                            <li>  
                                <form action="/admin/Rechargemange/index?parma=abcf&step=0" method="post" id="J-form">
                                                    ...
                                                    {foreach from=$res key=key item=data}
                                                        {if $data.id lt 30}
                                                            <label style="width:120px;">
                                                                <input type="checkbox" {if $data.canRechargeAppeal eq '1'}checked{/if} name="net_bank[]" value="{$data.id}" > {$data.name}
                                                            </label>
                                                            {if $key%4 eq '3'}
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {foreach from=$res key=key item=data}
                                                        {if $data.id lt 30}
                                                            <label style="width:120px;">
                                                                <input type="checkbox" {if $data.otherCanRechargeAppeal eq '1'}checked{/if} name="quick_bank[]" value="{$data.id}" > {$data.name}
                                                            </label>
                                                            {if $key%4 eq '3'}
                                                                <div class="clearfix"></div>
                                                            {/if}
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {foreach from=$res key=key item=data}
                                                        {if $data.id eq 31}
                                                            <label style="width:120px;">
                                                                <input type="checkbox" {if $data.canRechargeAppeal eq '1'}checked{/if} name="third_bank[]" value="{$data.id}" > {$data.name}
                                                            </label>
                                                        {/if}
                                                    {/foreach}
                                                </td>
